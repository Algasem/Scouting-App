const USERNAME_KEY = "ScoutUsername_v1";

const teamData = {
    "1": "The Juggernauts",
    "4": "Team 4 ELEMENT",
    "8": "Paly Robotics",
    "11": "MORT",
    "16": "Bomb Squad",
    "20": "The Rocketeers",
    "21": "ComBBAT",
    "22": "Double Deuce",
    "25": "Raider Robotix",
    "27": "Team RUSH",
    "28": "Pierson Whalers",
    "31": "Jenks Prime Movers",
    "33": "Killer Bees",
    "34": "Rockets",
    "41": "RoboWarriors",
    "45": "TechnoKats Robotics Team",
    "48": "Team E.L.I.T.E.",
    "51": "Wings of Fire",
    "56": "R.O.B.B.E.",
    "58": "The Riot Crew",
    "59": "RamTech",
    "60": "Kingman FIRST Robotics Team 60 (The Bionic Bulldogs)",
    "61": "BVT Robotics",
    "63": "McDowell Robotics Team 63",
    "66": "Grizzly Robotics",
    "67": "The HOT Team",
    "68": "Truck Town Thunder",
    "69": "HYPER",
    "70": "More Martians",
    "71": "Team Hammond",
    "74": "Team CHAOS",
    "75": "RoboRaiders",
    "78": "AIR STRIKE",
    "79": "Team 79 Krunch",
    "85": "B.O.B. (Built on Brains)",
    "86": "Team Resistance",
    "87": "Diablo",
    "88": "TJ²",
    "93": "NEW Apple Corps",
    "94": "The TechnoJays",
    "95": "Grasshoppers",
    "97": "Bionic Beef",
    "100": "The Wildhats",
    "102": "The Gearheads",
    "103": "Cybersonics",
    "107": "Team R.O.B.O.T.I.C.S.",
    "108": "SigmaC@T Robotics Team",
    "111": "WildStang",
    "112": "Plus One",
    "114": "Eaglestrike",
    "115": "MVRT",
    "116": "Epsilon Delta",
    "117": "Steel Dragons",
    "118": "Robonauts",
    "120": "Youth Tech Academy Red Dragons",
    "122": "NASA Knights",
    "123": "Team - Cosmos",
    "125": "NUTRONs",
    "126": "Gael Force",
    "127": "Bluetonium",
    "131": "CHAOS",
    "133": "B.E.R.T.",
    "135": "Penn Robotics Black Knights",
    "138": "Entropy",
    "141": "WOBOT",
    "144": "Operation Orange",
    "148": "Robowranglers",
    "151": "Tough Techs",
    "155": "The TechnoNuts",
    "157": "AZTECHS",
    "159": "Alpine Robotics",
    "166": "Chop Shop",
    "167": "Children of the Corn",
    "171": "Cheese Curd Herd",
    "172": "Northern Force",
    "173": "RAGE Robotics ⚙️",
    "175": "Buzz Robotics",
    "176": "Aces High",
    "177": "Bobcat Robotics",
    "178": "The 2nd Law Enforcers",
    "179": "Children of the Swamp",
    "180": "S.P.A.M.",
    "181": "Birds Of Prey",
    "188": "Blizzard",
    "190": "Gompei and the HERD",
    "191": "X-CATS",
    "192": "GRT",
    "193": "MORT Beta",
    "195": "CyberKnights",
    "199": "Deep Blue",
    "201": "The FEDS",
    "203": "SOUPERBOTS",
    "204": "Eastern Robotic Vikings",
    "217": "ThunderChickens",
    "219": "Team Impact",
    "222": "Tigertrons",
    "223": "Xtreme Heat",
    "226": "Hammerheads",
    "228": "GUS",
    "229": "Division By Zero",
    "230": "Gaelhawks",
    "231": "High Voltage",
    "233": "The Pink Team",
    "234": "Cyber Blue",
    "236": "Techno-Ticks",
    "237": "Black Magic Robotics",
    "238": "Crusaders",
    "240": "TEMPEST",
    "244": "RoboDawgs 3D",
    "245": "Adambots",
    "246": "Lobstah Bots",
    "247": "Da Bears",
    "250": "The Dynamos",
    "253": "Boba Bots",
    "254": "The Cheesy Poofs",
    "263": "Aftershock",
    "271": "Mechanical Marauders",
    "272": "Cyber Crusaders",
    "281": "The GreenVillains",
    "287": "Floyd Robotics",
    "288": "The RoboDawgs",
    "291": "CIA - Creativity In Action",
    "292": "PantherTech",
    "293": "SPIKE",
    "294": "Beach Cities Robotics",
    "296": "Northern Knights",
    "302": "The Dragons",
    "303": "The T.E.S.T. Team",
    "308": "Monsters",
    "314": "The Megatron Oracles",
    "316": "LUNATECS",
    "319": "Big Bad Bob",
    "321": "RoboLancers",
    "324": "Chips",
    "325": "Respawn Robotics",
    "328": "Penn Robotics Golden Rooks",
    "329": "Mohawks",
    "333": "MEGALODONS",
    "334": "TechKnights",
    "335": "Skillz Tech Gear Botz",
    "339": "Kilroy Robotics",
    "340": "G.R.R. (Greater Rochester Robotics)",
    "341": "Miss Daisy",
    "342": "Burning Magnetos",
    "343": "Metal-In-Motion",
    "346": "RoboHawks",
    "348": "Norwell Robotics",
    "353": "POBots",
    "354": "G-House Pirates",
    "358": "Robotic Eagles",
    "359": "Hawaiian Kids",
    "360": "The Revolution",
    "364": "Team Fusion 364",
    "365": "Miracle Workerz",
    "369": "High Voltage Robotics",
    "378": "The Circuit Stompers",
    "379": "The RoboCats",
    "386": "Team Voltage",
    "395": "2 Train Robotics",
    "399": "Eagle Robotics",
    "401": "Copperhead Robotics",
    "404": "⚒️ 404 ART Not Found ⚒️",
    "418": "Purple Haze",
    "422": "The Mech Tech Dragons",
    "423": "Simple Machines",
    "424": "Rust Belt Robotics",
    "427": "LANCE-A-LOT",
    "428": "BoroBlasters",
    "429": "Two Many Bobcats",
    "430": "MORT GAMMA",
    "431": "PinkZilla",
    "432": "The Final Countdown",
    "433": "Firebirds",
    "435": "Robodogs",
    "436": "Purple Rain",
    "447": "Team Roboto",
    "449": "The Blair Robot Project",
    "451": "Cat Attack Robotics",
    "453": "Rockem Sockem Robotics",
    "456": "Siege Robotics",
    "457": "⚙️ Grease Monkeys ⚙️",
    "461": "Westside Boiler Invasion",
    "467": "Center of Mass",
    "469": "Las Guerrillas",
    "484": "Roboforce",
    "486": "Positronic Panthers",
    "488": "Team XBOT",
    "492": "Titan Robotics Club",
    "494": "Martians",
    "496": "Royals",
    "498": "The Cobra Commanders",
    "501": "The PowerKnights",
    "503": "Frog Force",
    "509": "Red Storm",
    "514": "Entropy",
    "516": "StuyPlus",
    "525": "Swartdogs",
    "526": "Saxon Sparks",
    "527": "Red Dragons",
    "533": "The PSIcotics",
    "537": "Charger Robotics",
    "538": "Renaissance Robotics",
    "539": "Titan Robotics",
    "540": "TALON 540 Godwin Robotics",
    "545": "ROBO-DAWGS",
    "547": "Falcon Engineering And Robotics",
    "548": "Robostangs",
    "554": "The Blue Grease Crew",
    "555": "Montclair Robotics",
    "558": "Elm City Robo Squad",
    "564": "Longwood RoboLions",
    "569": "Rambots 569",
    "571": "Paragon Robotics",
    "573": "Mech Warriors",
    "578": "Fairport Robotics",
    "581": "Blazing Bulldogs",
    "587": "The Hedgehogs",
    "589": "Falkon Robotics",
    "590": "Chahta Warriors",
    "597": "The Wolverines",
    "599": "The Robodox",
    "601": "BayBots",
    "604": "Quixilver",
    "610": "Crescent Coyotes",
    "611": "Saxons",
    "612": "Chantilly Robotics",
    "614": "Night Hawks",
    "617": "Enginerds",
    "619": "Cavalier Robotics",
    "620": "Warbots",
    "623": "Oakton Cougar Robotics",
    "624": "CRyptonite",
    "639": "Code Red Robotics",
    "640": "Edison Robotics",
    "648": "QC ELITE Flaming Squirrels",
    "649": "MSET Fish",
    "662": "Rocky Mountain Robotics",
    "663": "Power Sharks",
    "668": "The Apes of Wrath",
    "670": "Homestead Robotics",
    "677": "Murphy's Outlaws",
    "686": "Bovine Intervention",
    "687": "The Nerd Herd",
    "691": "Project 691 Robotics",
    "694": "StuyPulse",
    "695": "Bison Robotics",
    "696": "Circuit Breakers",
    "698": "Hamilton Microbots",
    "701": "RoboVikes",
    "702": "Bagel Bytes",
    "706": "Cyberhawks",
    "708": "Hatters Robotics",
    "709": "Femme Tech Fatale",
    "714": "Panthera",
    "716": "Who'sCTEKS",
    "743": "Technobots",
    "744": "Shark Attack",
    "749": "Ctrl V",
    "751": "barn2robotics",
    "752": "Chargers",
    "753": "High Desert Droids",
    "757": "Crazy Train",
    "761": "TechPUPS",
    "766": "M-A Bears",
    "771": "SWAT",
    "772": "Sabre Bytes Robotics",
    "781": "Kinetic Knights",
    "801": "Horsepower",
    "806": "The Brooklyn Blacksmiths",
    "810": "The Mechanical Bulls",
    "811": "Wild Cards",
    "812": "The Midnight Mechanics",
    "815": "Advanced Power",
    "816": "Anomaly",
    "818": "The Steel Armadillos",
    "829": "The Digital Goats",
    "830": "The RatPack",
    "832": "OSCAR",
    "834": "SparTechs",
    "836": "The RoboBees",
    "839": "Rosie Robotics",
    "840": "Aragon Robotics Team",
    "841": "The BioMechs",
    "846": "The Funky Monkeys",
    "847": "PHRED",
    "852": "ARC Robotics",
    "854": "Iron Bears",
    "857": "Superior Roboworks",
    "858": "Demons Robotics",
    "862": "Lightning Robotics",
    "865": "WARP7",
    "867": "Absolute Value",
    "868": "TechHOUNDS",
    "870": "TEAM R. I. C. E.",
    "871": "Robotechs",
    "876": "Thunder Robotics",
    "877": "Pi Rho 'botics @ North Star",
    "884": "Mechanical Mules",
    "888": "Robotiators",
    "894": "The Power Chargers",
    "900": "The Zebracorns",
    "904": "D Cubed",
    "910": "The Foley Freeze",
    "919": "PANTHERA TIGRIS",
    "930": "Mukwonago BEARs",
    "931": "Perpetual Chaos",
    "935": "RaileRobotics",
    "937": "Nerd Herd",
    "948": "NRG (Newport Robotics Group)",
    "949": "Wolverine Robotics",
    "955": "Ctrl C",
    "957": "SWARM - South and West Albany Robotics Maniacs",
    "967": "Iron Lions",
    "968": "RAWC (Robotics Alliance Of West Covina)",
    "971": "Spartan Robotics",
    "972": "Iron Claw",
    "973": "Greybots",
    "977": "Cometbots",
    "980": "ThunderBots",
    "987": "HIGHROLLERS",
    "988": "Steel Phoenix",
    "991": "BroncoBots",
    "996": "Mecha Knights",
    "997": "Spartan Robotics",
    "999": "MechaRAMS",
    "1002": "CircuitRunners Robotics",
    "1011": "CRUSH",
    "1014": "Bad Robots",
    "1018": "Pike RoboDevils",
    "1023": "Bedford Express",
    "1024": "Kil-A-Bytes",
    "1025": "Impi Warriors",
    "1027": "Mechatronic Maniacs",
    "1038": "Lakota Robotics",
    "1051": "Technical Terminators",
    "1056": "Hot Rocks",
    "1058": "PVC Pirates",
    "1071": "Team MAX",
    "1072": "Harker Robotics",
    "1073": "The Force Team",
    "1076": "PiHi Samurai",
    "1086": "Blue Cheese",
    "1089": "Team Mercury",
    "1091": "Oriole Assault",
    "1099": "DiscoTechs",
    "1100": "Inverse Polarity",
    "1102": "M'Aiken Magic",
    "1108": "Panther Robotics",
    "1111": "Power Hawks Robotics",
    "1114": "Simbotics",
    "1119": "Pup Cups",
    "1126": "SPARX",
    "1138": "Eagle Engineering",
    "1148": "Harvard Westlake Robotics",
    "1153": "Timberwolves",
    "1155": "SciBorgs",
    "1156": "Under Control",
    "1159": "Ramona Rampage",
    "1160": "Titanium Robotics",
    "1164": "Project NEO",
    "1165": "Team Paradise",
    "1168": "Malvern Robotics",
    "1178": "DURT (De Smet United Robotics Team)",
    "1188": "The Ravens",
    "1189": "Gearheads",
    "1197": "TorBots",
    "1208": "Metool Brigade",
    "1209": "RoboHornets",
    "1212": "Sentinels",
    "1218": "SCH Robotics",
    "1220": "Rockhoppers - Hilltopper Robotics",
    "1225": "The Gorillas",
    "1229": "Tidal Shift",
    "1241": "THEORY6",
    "1247": "Blood, Sweat, and Gears",
    "1250": "Gator-Bots",
    "1255": "Blarglefish",
    "1257": "Parallel Universe",
    "1259": "Paradigm Shift",
    "1261": "Robo Lions Team1261",
    "1262": "the STAGS",
    "1277": "The Robotomies",
    "1279": "Cold Fusion",
    "1280": "Ragin' C-Biscuits",
    "1287": "Aluminum Assault",
    "1288": "RAVEN Robotics",
    "1293": "Pandamaniacs",
    "1294": "Pack of Parts",
    "1296": "Full Metal Jackets",
    "1303": "WYOHAZARD",
    "1305": "Ice Cubed",
    "1306": "BadgerBOTS",
    "1307": "Robosaints",
    "1308": "Wildcat Robotics",
    "1310": "Runnymede Robotics",
    "1317": "Digital Fusion",
    "1318": "Issaquah Robotics Society",
    "1319": "Flash",
    "1322": "GRAYT Leviathons (Genesee Robotics Area Youth Team)",
    "1323": "MadTown Robotics",
    "1325": "Inverse Paradox",
    "1329": "The ViPrs",
    "1334": "OT Devils",
    "1339": "Angelbotics",
    "1350": "The Rambots",
    "1351": "TKO",
    "1359": "Scalawags",
    "1360": "Orbit Robotics",
    "1369": "Minotaur",
    "1388": "Eagle Robotics",
    "1389": "The Body Electric",
    "1391": "The Metal Moose",
    "1403": "Team 1403 Cougar Robotics",
    "1405": "Finney Falcons",
    "1410": "The Kraken",
    "1414": "IHOT",
    "1418": "Vae Victis",
    "1421": "Team CHAOS",
    "1425": "Error Code Xero",
    "1445": "Webb Spark",
    "1446": "Robo Knights",
    "1450": "XQ Robotix Team 1450",
    "1452": "Omnicats",
    "1458": "Red Tie Robotics",
    "1466": "Webb Robotics",
    "1468": "Hicksville J-Birds",
    "1474": "Titans",
    "1477": "Texas Torque",
    "1481": "The Riveters",
    "1493": "Falcons",
    "1498": "The Polar Pilots",
    "1501": "Team THRUST",
    "1502": "Technical Difficulties",
    "1504": "Desperate Penguins",
    "1506": "Metal Muscle",
    "1507": "Warlocks",
    "1511": "Rolling Thunder",
    "1512": "The Big Red",
    "1515": "MorTorq",
    "1518": "Raider Robotics",
    "1522": "DOTM - Defenders of the Multiverse",
    "1523": "MARS (Mega Awesome Robotic Systems)",
    "1528": "MTR (Monroe Trojan Robotics)",
    "1533": "Triple Strange",
    "1538": "The Holy Cows",
    "1540": "Flaming Chickens",
    "1546": "Chaos, Inc.",
    "1554": "Oceanside Kraken",
    "1555": "CryoFrost",
    "1557": "12 Volt Bolt",
    "1559": "Devil Tech",
    "1561": "Roboducks",
    "1566": "AMMOKNIGHTS",
    "1569": "Haywire Robotics",
    "1574": "MisCar",
    "1576": "Voltrix",
    "1577": "Steampunk",
    "1580": "The Blue Monkeys",
    "1585": "Scitobor Robotics",
    "1591": "Greece Gladiators",
    "1592": "Bionic Tigers",
    "1595": "The Dragons",
    "1596": "The Instigators",
    "1599": "CircuiTree",
    "1601": "Quantum Samurai",
    "1619": "Up-A-Creek Robotics",
    "1622": "Team Spyder",
    "1625": "Winnovation",
    "1626": "Falcon Robotics",
    "1629": "Garrett Coalition (GaCo)",
    "1635": "TECHNOTICS",
    "1640": "Sab-BOT-age",
    "1646": "Precision Guessworks",
    "1647": "Iron Devils",
    "1648": "G3 Robotics",
    "1661": "Griffitrons",
    "1671": "Buchanan Bird Brains",
    "1672": "Robo T-Birds",
    "1675": "UPS (Ultimate Protection Squad)",
    "1676": "The Pascack PI-oneers",
    "1678": "Citrus Circuits",
    "1683": "Techno Titans",
    "1684": "The Chimeras",
    "1690": "Orbit",
    "1699": "Robocats",
    "1700": "Gatorbotics",
    "1701": "Robocubs",
    "1706": "Ratchet Rockers",
    "1708": "AMP'D Robotics",
    "1710": "The Ravonics Revolution",
    "1711": "RAPTORS",
    "1712": "Dawgma",
    "1714": "MORE Robotics",
    "1716": "Redbird Robotics",
    "1718": "The Fighting Pi",
    "1719": "The Umbrella Corporation",
    "1720": "PhyXTGears",
    "1721": "Tidal Force",
    "1723": "The FBI - FIRST Bots of Independence",
    "1726": "N.E.R.D.S. (Nifty Engineering Robotics Design Squad)",
    "1727": "REX",
    "1729": "Team Inconceivable!",
    "1730": "Team Driven",
    "1731": "Fresta Valley Robotics Club",
    "1732": "Hilltopper Robotics",
    "1735": "Green Reapers",
    "1736": "Robot Casserole",
    "1739": "Chicago Knights Robotics",
    "1740": "Cyber Colonels",
    "1741": "Red Alert",
    "1744": "Robo-Rays Robotics",
    "1745": "The P-51 Mustangs",
    "1746": "Team OTTO",
    "1747": "Harrison Boiler Robotics",
    "1756": "Argos",
    "1757": "Wolverines",
    "1758": "Technomancers",
    "1759": "Potatoes",
    "1761": "STEAMpunk Tigers",
    "1763": "Paséliens",
    "1764": "Liberty Robotics",
    "1768": "Nashoba Robotics",
    "1769": "Digital Hawks",
    "1771": "North Gwinnett Robotics",
    "1772": "The Brazilian Trail Blazers",
    "1775": "Tigerbytes",
    "1778": "Chill Out",
    "1781": "Lindblom Electric Eagles",
    "1785": "Blue Springs Robocats",
    "1787": "The Flying Circuits",
    "1792": "Round Table Robotics",
    "1793": "The Aviators",
    "1795": "Carver Clutch",
    "1796": "RoboTigers",
    "1799": "Wired-Up!",
    "1802": "Team Stealth",
    "1803": "Vikings",
    "1806": "S.W.A.T.",
    "1807": "Redbird Robotics",
    "1810": "CATATRONICS",
    "1811": "FRESH",
    "1816": "\"The Green Machine\"",
    "1818": "Team Talos",
    "1819": "The Quacken",
    "1822": "Electric Calamari",
    "1825": "The Cyborgs",
    "1827": "THE HIVE",
    "1828": "BoxerBots",
    "1832": "Duck Tape",
    "1833": "Team BEAN",
    "1836": "The MilkenKnights",
    "1847": "WRATH",
    "1854": "The Chomp Bots",
    "1857": "Citrus Sparks",
    "1860": "Alphabots",
    "1868": "Space Cookies",
    "1880": "Warriors of East Harlem",
    "1884": "Griffins",
    "1885": "Comet Robotics",
    "1891": "Bullbots",
    "1895": "Lambda Corps",
    "1899": "Saints Robotics",
    "1902": "Exploding Bacon",
    "1908": "ShoreBots",
    "1912": "Team Combustion",
    "1915": "Firebird Robotics",
    "1918": "NC GEARS",
    "1922": "Oz-Ram",
    "1923": "The MidKnight Inventors",
    "1927": "Tempest",
    "1937": "Elysium",
    "1939": "THE KUHNIGHTS",
    "1940": "Tech Tigers 1940",
    "1942": "Cinderella Tel-Nof",
    "1943": "Neat Team",
    "1954": "ElectroBunny",
    "1967": "The Janksters",
    "1972": "Searing Engineering",
    "1977": "The PowerSquids",
    "1982": "Cougar Robotics",
    "1983": "Skunk Works Robotics",
    "1986": "Team Titanium",
    "1987": "Broncobots",
    "1991": "The Dragons",
    "1994": "Quixote",
    "1997": "STAGROBOTICS",
    "2001": "HERMES",
    "2010": "The Lightning Bots",
    "2013": "Cybergnomes",
    "2016": "Mighty Monkey Wrenches",
    "2022": "Titan Robotics",
    "2027": "Robotic Dragons",
    "2028": "Phantom Robotics",
    "2035": "The Rockin' Bots",
    "2036": "Boulder Valley Black Knights",
    "2039": "Rockford Robotics",
    "2040": "DERT - Dunlap Eagles Robotics Team",
    "2046": "Bear Metal",
    "2048": "The Pink Panthers",
    "2052": "KnightKrawler",
    "2053": "Southern Tier Robotics",
    "2054": "Tech Vikes",
    "2056": "OP Robotics",
    "2059": "The Hitchhikers",
    "2062": "CORE 2062",
    "2064": "The Panther Project",
    "2067": "Apple Pi",
    "2068": "Metal Jackets",
    "2073": "EagleForce",
    "2075": "Enigma Robotics",
    "2077": "Laser Robotics 2077",
    "2079": "4H ALARM Robotics",
    "2080": "Torbotics",
    "2083": "Team Blitz",
    "2084": "Robots by the C",
    "2090": "Buff 'n Blue",
    "2095": "Direct Current",
    "2096": "RoboActive",
    "2102": "Team Paradox",
    "2106": "The Junkyard Dogs",
    "2122": "Team Tators",
    "2129": "Ultraviolet",
    "2130": "Alpha+",
    "2135": "Presentation Invasion",
    "2137": "The Oxford RoboCats",
    "2141": "Spartronics",
    "2143": "Team Tobor",
    "2145": "HAZMATS",
    "2147": "CHUCK",
    "2151": "PMSA Monty Pythons",
    "2152": "S*M*A*S*H",
    "2158": "the ausTIN CANs",
    "2161": "Robocats",
    "2165": "BisonBots",
    "2168": "Aluminum Falcons",
    "2169": "KING TeC",
    "2170": "Galvanized Guardians",
    "2171": "RoboDogs",
    "2172": "Steel Unit-E",
    "2175": "The Fighting Calculators",
    "2177": "The Robettes",
    "2180": "Zero Gravity",
    "2181": "GEARS",
    "2183": "Tigerbots",
    "2186": "Dogs of Steel",
    "2191": "Flux Core",
    "2194": "Fondy Fire",
    "2197": "Las Pumas",
    "2199": "Robo-Lions",
    "2200": "BCR Blackout",
    "2202": "BEAST Robotics",
    "2204": "Rambots",
    "2207": "Bright Bears",
    "2212": "The Spikes",
    "2220": "Blue Twilight",
    "2221": "FHS Robodawgs",
    "2224": "RoboPhoenix",
    "2225": "Rebel Robotics",
    "2227": "Tigers",
    "2228": "CougarTech",
    "2230": "General Angels",
    "2231": "OnyxTronix",
    "2234": "Alternating Current",
    "2239": "Technocrats",
    "2240": "Brute Force",
    "2252": "The Mavericks",
    "2259": "1/4 Twenties",
    "2261": "Casa Robotics",
    "2262": "Robo-Panthers",
    "2264": "Trojan Robotics",
    "2265": "Fe Maidens",
    "2283": "Panteras",
    "2288": "Hydra Robotics",
    "2290": "FLYT",
    "2333": "Sapulpa Robotics Team",
    "2337": "EngiNERDs",
    "2338": "Gear It Forward",
    "2340": "Mercy Robotics-Xcentrics",
    "2341": "Sprockets",
    "2342": "Team Phoenix",
    "2344": "The Saunders Droid Factory",
    "2345": "Kearnage",
    "2347": "Trial and Error",
    "2348": "Mechahunes",
    "2352": "Metal Mayhem",
    "2357": "System Meltdown",
    "2358": "Bearbotics",
    "2359": "RoboLobos",
    "2363": "Triple Helix Robotics",
    "2367": "Lancer Robotics",
    "2370": "IBOTS",
    "2373": "The Crickets",
    "2374": "Jesuit Robotics",
    "2375": "Dragon Robotics",
    "2377": "C Company",
    "2383": "Ninjineers",
    "2386": "Trojans",
    "2393": "Robotichauns",
    "2399": "The Fighting Unicorns",
    "2403": "Plasma Robotics",
    "2404": "Team NEY Techs (TNT)",
    "2405": "Techno Trojans",
    "2408": "Shrapnel Sergeants",
    "2410": "Blue Valley CAPS Metal Mustangs",
    "2412": "Robototes",
    "2415": "WiredCats",
    "2421": "RTR Team Robotics",
    "2423": "The KwarQs",
    "2429": "The Blockheads",
    "2438": "'Iobotics",
    "2441": "Spartechs",
    "2443": "Blue Thunder",
    "2445": "RoboKAP",
    "2450": "Wind Chill",
    "2457": "The Law",
    "2458": "Team Chaos",
    "2465": "KauaiBots",
    "2468": "Team Appreciate",
    "2470": "Team BJORG",
    "2471": "Team Mean Machine",
    "2472": "Centurions",
    "2473": "Goldstrikers",
    "2477": "Marauder Rascals",
    "2478": "Westwood Robotics",
    "2480": "Iron Paws",
    "2481": "Roboteers",
    "2485": "Overclocked",
    "2486": "CocoNuts",
    "2487": "Mechanical Animals",
    "2489": "The Insomniacs",
    "2491": "NoMythic",
    "2495": "Hive Mind",
    "2498": "BearBotics",
    "2500": "Herobotics",
    "2501": "Bionic Polars",
    "2502": "Talon Robotics",
    "2503": "Warrior Robotics",
    "2506": "Saber Robotics",
    "2508": "Armada",
    "2509": "Tigerbots",
    "2511": "Cougars",
    "2512": "Duluth East Daredevils",
    "2513": "T.E.R.T.O.L.A.",
    "2518": "Spartan Robotics",
    "2521": "SERT",
    "2522": "Royal Robotics",
    "2523": "Tech Storm",
    "2526": "Crimson Robotics",
    "2530": "Inconceivable",
    "2531": "RoboHawks",
    "2534": "Lumberjack Robotics",
    "2537": "Space RAIDers",
    "2538": "The Plaid Pillagers",
    "2539": "Krypton Cougars",
    "2543": "TitanBot",
    "2549": "Millerbots",
    "2550": "Skynet",
    "2551": "Penguin Empire",
    "2554": "The Warhawks",
    "2556": "RadioActive Roaches",
    "2557": "SOTAbots",
    "2559": "Normality Zero",
    "2560": "RoboDog",
    "2574": "RoboHuskie",
    "2577": "Pingry Robotics",
    "2582": "PantherBots",
    "2583": "Orange Dynamite",
    "2584": "Flame of The West",
    "2585": "Impact",
    "2586": "Copper Bots",
    "2587": "DiscoBots",
    "2590": "Nemesis",
    "2591": "RedTails",
    "2594": "Naskcorpions",
    "2601": "Steel Hawks",
    "2603": "Steel Stingers",
    "2604": "Metal and Soul",
    "2606": "Irish Robotics",
    "2607": "The Fighting RoboVikings",
    "2609": "Beaverworx",
    "2611": "Jacktown Vectors",
    "2614": "Mountaineer Area RoboticS (MARS)",
    "2619": "The Charge",
    "2620": "Titans",
    "2626": "Evolution",
    "2630": "Thunderbolts",
    "2634": "The Gryphons",
    "2635": "Lake Monsters",
    "2637": "Phantom Catz",
    "2638": "Rebel Robotics",
    "2640": "HOTBOTZ",
    "2641": "Pittsburgh Central Catholic Robotics",
    "2643": "Dark Matter",
    "2648": "Infinite Loop",
    "2654": "Polar Rams",
    "2656": "Quasics",
    "2657": "Team Thundercats",
    "2658": "Σ-Motion",
    "2659": "RoboWarriors",
    "2662": "RoboKrew",
    "2667": "Knights of the Valley",
    "2673": "Tenacious Technicians",
    "2679": "Atlantis",
    "2682": "Boneyard Robotics 2682",
    "2687": "Team Apprentice",
    "2689": "Team Alpha",
    "2702": "Rebels",
    "2704": "Roaring Robotics",
    "2706": "Merge Robotics",
    "2708": "Lake Effect Robotics",
    "2710": "JetStream",
    "2712": "Power Surge 4-H Robotics",
    "2713": "Red Hawk Robotics",
    "2714": "♨️ BBQ ♨️",
    "2718": "Team OKC e'possums (Σ 1/n! x 10³)",
    "2720": "Red Watch Robotics",
    "2722": "Charge Robotics",
    "2724": "Avalanche Robotics",
    "2726": "Red Pebble Rebels",
    "2728": "♨️ TACO ♨️",
    "2733": "Pigmice",
    "2767": "Stryke Force",
    "2771": "Code Red Robotics the Stray Dogs",
    "2773": "IronDogz",
    "2783": "Engineers of Tomorrow",
    "2785": "Prometheus",
    "2789": "TEXPLOSION",
    "2791": "Shaker Robotics",
    "2797": "Knight & Nerdy",
    "2811": "StormBots",
    "2813": "Gear Heads",
    "2815": "Blue Devil Mechanics",
    "2823": "The Automatons",
    "2826": "Wave Robotics",
    "2827": "The Tiki Bots",
    "2830": "Riverside RoboTigers",
    "2832": "The Livonia Warriors",
    "2834": "Bionic Black Hawks",
    "2839": "Daedalus",
    "2840": "PCDS Blue Tide",
    "2846": "FireBears",
    "2847": "The MegaHertz",
    "2848": "Rangers",
    "2849": "Ursa Major",
    "2851": "Crevolution",
    "2852": "DM High Voltage",
    "2853": "Hot Spot Robotics",
    "2854": "The Prototypes",
    "2855": "BEASTBOT",
    "2856": "Planetary Drive",
    "2861": "Infinity's End",
    "2867": "ElkLogics",
    "2869": "Regal Eagles",
    "2872": "CyberCats",
    "2875": "CyberHawks",
    "2876": "DevilBotz",
    "2877": "LigerBots",
    "2881": "Lady Cans",
    "2882": "Nuts n' Boltz",
    "2883": "F.R.E.D (FIRST Robotics Engineering and Design)",
    "2890": "The Hawk Collective",
    "2896": "MechaMonarchs",
    "2898": "Flying Hedgehogs",
    "2900": "The Mighty Penguins",
    "2903": "NeoBots",
    "2905": "Sultans of Turkiye",
    "2906": "Sentinel Prime Robotics",
    "2910": "Jack in the Bot",
    "2912": "Panther Robotics",
    "2914": "TIGER PRIDE",
    "2915": "Pandamonium",
    "2926": "Robo Sparks",
    "2927": "NEST Robotics",
    "2928": "Viking Robotics",
    "2929": "Emeral Ridge JAGBOTS",
    "2930": "Sonic Squirrels",
    "2935": "NACI Robotics",
    "2945": "Steel Mustangs",
    "2950": "Devastators",
    "2959": "Robotarians",
    "2960": "Automation Nation",
    "2963": "AnaDroids",
    "2969": "Aftermath",
    "2972": "Gears and Buccaneers",
    "2973": "Mad Rockers",
    "2974": "Walton Robotics",
    "2976": "The Spartabots",
    "2977": "Sir LancerBots",
    "2978": "Cavaliers",
    "2980": "Whidbey Island Wildcats",
    "2984": "Vikings Robotics",
    "2987": "Rogue Robotics",
    "2988": "RoboRiot",
    "2990": "Hotwire",
    "2992": "The S.S. Prometheus",
    "2996": "Cougars Gone Wired",
    "2998": "VikingBots",
    "3003": "TAN(X)",
    "3004": "Bx Quixote",
    "3005": "RoboChargers",
    "3006": "Red Rock Robotics",
    "3008": "Team Magma",
    "3009": "High Scalers",
    "3015": "Ranger Robotics",
    "3017": "Patriots",
    "3018": "Nordic Storm",
    "3019": "Firebirds",
    "3020": "SMbld",
    "3023": "STARK Industries",
    "3024": "My Favorite Team",
    "3026": "Orange Crush Robotics",
    "3035": "Droid Rage",
    "3038": "Inter-district Computers and Engineering",
    "3039": "Wildcat Robotics",
    "3042": "Cobalt Catalysts",
    "3044": "Team 0xBE4",
    "3045": "Gear Gremlins",
    "3049": "BremerTron",
    "3053": "VB Stingers",
    "3054": "IceStorm",
    "3055": "Furious George",
    "3061": "Huskie Robotics",
    "3065": "Jatt High School",
    "3067": "Robovikes",
    "3070": "Team Pronto 3070",
    "3072": "Backwoods Bots",
    "3075": "Ha-Dream Team",
    "3082": "Chicken Bot Pie",
    "3083": "Artemis",
    "3090": "Chaotic Robotics",
    "3091": "100 Scholars",
    "3096": "The Village Bulldogs",
    "3098": "Waterford Robotics",
    "3100": "The Lightning Turtles",
    "3102": "Tech-No-Tigers",
    "3117": "Harfangs",
    "3128": "The Aluminum Narwhals",
    "3130": "The ERRORs",
    "3132": "Thunder Down Under",
    "3134": "The Accelerators",
    "3136": "O.R.C.A.",
    "3137": "T-Birds",
    "3138": "Innovators Robotics",
    "3140": "Flagship",
    "3142": "Aperture",
    "3146": "GRANBY GRUNTS",
    "3147": "Munster Horsepower",
    "3158": "PrepaTec - TECBOT",
    "3160": "F. R. O. G. - FIRST Robotics Organization of Grove",
    "3161": "Tronic Titans",
    "3163": "ROARBOTS",
    "3164": "Stealth Tigers",
    "3166": "Rooks",
    "3171": "HURRICANES",
    "3173": "IgKnighters",
    "3175": "Knight Vision",
    "3176": "Purple Precision",
    "3179": "The A.N.T.",
    "3181": "Pittsford Panthers",
    "3182": "Athena's Warriors",
    "3184": "Blaze Robotics",
    "3189": "Circuit Breakers",
    "3193": "Falco Tech",
    "3196": "Team SPORK",
    "3197": "HexHounds",
    "3200": "Raptacon",
    "3201": "Ross Rambotics",
    "3204": "Steampunk Penguins",
    "3205": "Patriots",
    "3206": "Royal T-Wrecks",
    "3211": "The Y Team",
    "3212": "YME Stingbots",
    "3216": "MRT",
    "3218": "Panther Robotics",
    "3219": "TREAD (Trojan Robotics, Engineering, Art, & Design)",
    "3229": "Hawktimus Prime",
    "3234": "Red Arrows",
    "3240": "Team Orion",
    "3242": "ILLUMICATS",
    "3243": "Amperes",
    "3245": "Ravens",
    "3247": "Robopack",
    "3255": "SuperNURDs",
    "3256": "WarriorBorgs",
    "3257": "Vortechs",
    "3258": "MADAWGS",
    "3260": "SHARP",
    "3267": "Mariner Robotics",
    "3268": "Valhallabots",
    "3275": "The Regulators",
    "3276": "TOOLCATS",
    "3277": "ProDigi",
    "3278": "Electric Lakers",
    "3282": "Dallas Robo Tigers",
    "3284": "Camdenton LASER 3284",
    "3288": "Punchers",
    "3291": "Au Pirates",
    "3293": "OtterBots",
    "3294": "Backwoods Engineers",
    "3295": "STEMdroids",
    "3297": "Full Metal Jackets",
    "3298": "ArrowBots",
    "3300": "Midwest WARRIORS",
    "3302": "TurboTrojans",
    "3309": "Friarbots",
    "3310": "Black Hawk Robotics",
    "3313": "Mechatronics",
    "3314": "Mechanical Mustangs",
    "3316": "D-Bug",
    "3318": "RoboDragons",
    "3322": "Eagle Evolution",
    "3323": "Potential Energy",
    "3324": "The Metrobots",
    "3328": "NoHoRobo",
    "3329": "Wildbots",
    "3336": "Zimanators",
    "3339": "BumbleB",
    "3340": "Union City EagleX Robotics",
    "3341": "Option 16",
    "3344": "Space Dragons",
    "3354": "PrepaTec - TecDroid",
    "3357": "COMETS",
    "3359": "Royal Robotics",
    "3360": "Hyperion",
    "3361": "Governators",
    "3373": "Team RoboHawk",
    "3381": "Droid Rage",
    "3388": "Flash in memory of Margarita Gusak",
    "3390": "ANATOLIAN EAGLEBOTS",
    "3393": "Horns of Havoc",
    "3397": "Robolions",
    "3407": "Wild Cards",
    "3408": "cyberCubs",
    "3414": "Hackbots",
    "3418": "RoboRiot",
    "3419": "RoHawks",
    "3424": "Corax",
    "3425": "PULSARS",
    "3452": "GreengineerZ",
    "3454": "Z Bots",
    "3458": "Code Blue",
    "3459": "Team PyroTech",
    "3461": "Operation PEACCE Robotics",
    "3464": "Sim-City",
    "3465": "The Omega Factor",
    "3467": "Windham Windup",
    "3468": "MAGNAtech",
    "3472": "PrepaTec - Buluk",
    "3473": "Team Sprocket",
    "3476": "Code Orange",
    "3478": "PrepaTec - LamBot",
    "3480": "PrepaTec - ABTOMAT",
    "3481": "Bronc Botz",
    "3482": "Arrowbotics",
    "3484": "Short Circuit",
    "3487": "Red Pride Robotics",
    "3488": "Eagle Army",
    "3489": "Category 5",
    "3490": "Viper Drive",
    "3492": "Putnam Area Robotics Team (P.A.R.T.s)",
    "3494": "The Quadrangles",
    "3501": "Firebots",
    "3502": "Octo(PI)Rates",
    "3504": "Girls of Steel",
    "3506": "YETI Robotics",
    "3507": "Ubotics #3507",
    "3512": "Spartatroniks",
    "3522": "CBoT´S4",
    "3526": "PrepaTec - Blue Ignition",
    "3527": "PrepaTec - Balam Esmeralda",
    "3532": "Piranhas",
    "3533": "Mekano",
    "3536": "Electro Eagles",
    "3537": "Delta Force",
    "3538": "RoboJackets",
    "3539": "Byting Bulldogs",
    "3543": "C4 Robotics",
    "3544": "Spartiates",
    "3545": "Bots in Blue",
    "3550": "Robotronix",
    "3559": "Thundercats",
    "3560": "MechaWolves",
    "3561": "RoboRaiders",
    "3566": "Gone Fishin'",
    "3568": "Linden RoboEagles",
    "3572": "Wavelength",
    "3574": "HIGH TEKERZ",
    "3577": "Saint's Robotics",
    "3596": "Rocket Robotics",
    "3597": "Robo-Rangers",
    "3598": "SEStematic Eliminators",
    "3602": "RoboMos",
    "3603": "Cyber Coyotes",
    "3604": "Goon Squad",
    "3606": "BearMetal",
    "3610": "Islanders",
    "3616": "Team Phenomena",
    "3618": "Petoskey Paladins",
    "3619": "PIONEERS",
    "3620": "Average Joes",
    "3623": "The Robomingos",
    "3624": "Soaring Colts",
    "3627": "The Jungle Robotics",
    "3630": "Stampede",
    "3632": "D-Techs",
    "3634": "Bridgeport Spark City",
    "3635": "Flying Legion",
    "3636": "Generals",
    "3637": "The Daleks",
    "3641": "The Flying Toasters",
    "3646": "INTEGRA BAHCESEHIR",
    "3647": "Millennium Falcons",
    "3648": "Sparta Robotica",
    "3654": "TechTigers",
    "3655": "Tractor Technicians",
    "3656": "Dexter Dreadbots",
    "3658": "Robo - Beavers",
    "3663": "CPR - Cedar Park Robotics",
    "3667": "Mecanum Knights",
    "3668": "TroBots",
    "3669": "CarbonKnights",
    "3673": "C.Y.B.O.R.G. Seagulls",
    "3674": "CloverBots",
    "3676": "RedShift Robotics",
    "3679": "rattlers",
    "3680": "Elemental Dragons",
    "3681": "Robo-Raiders",
    "3683": "Team DAVE",
    "3688": "Norsemen",
    "3691": "RoboRaiders",
    "3692": "Rock N' Robotics",
    "3695": "Foximus Prime",
    "3704": "RoboSapiens",
    "3707": "TechnoDogs",
    "3712": "RoboCats",
    "3714": "Flowers S.O.A.R.",
    "3719": "STEM Whalers",
    "3721": "Charger Robotics",
    "3723": "TEKnights",
    "3728": "Momentum",
    "3729": "The Raiders",
    "3734": "StarBots",
    "3735": "VorTX",
    "3739": "Oakbotics",
    "3740": "Storm Robotics",
    "3745": "Governors",
    "3748": "Ragnarok Robotics",
    "3749": "Team Optix",
    "3751": "Tower View Robotics",
    "3753": "BulahBots!",
    "3756": "RamFerno",
    "3759": "SMARTBOTS",
    "3760": "Robo Raccoons",
    "3767": "TC Titans",
    "3770": "BlitzCreek",
    "3773": "Arrowhead Robotics",
    "3777": "Bean City Bots",
    "3786": "Chargers",
    "3792": "Army Ants",
    "3794": "Prepa Tecmilenio WinT",
    "3799": "Electric Fire",
    "3807": "Overland BlazerBots",
    "3814": "PiBotics",
    "3815": "NF Raiders",
    "3821": "Pirabots",
    "3822": "Neon Krakens",
    "3824": "HVA RoHAWKtics",
    "3826": "Sequim Robotics Federation \"SRF\"",
    "3835": "Vulcan",
    "3838": "Roc City Robotix",
    "3843": "ROBO RACERS",
    "3847": "Spectrum -△◅",
    "3848": "Bots in Shining Armour",
    "3863": "Pantherbotics",
    "3865": "Riley WildBots",
    "3871": "Trojan Robotics #3871",
    "3875": "Red Storm Robotics",
    "3876": "Mabton LugNutz",
    "3880": "Tiki Techs",
    "3881": "WHEA Sharkbots",
    "3882": "Lunas",
    "3883": "Data Bits",
    "3891": "Rodeo Robotics",
    "3892": "Roboponies",
    "3893": "K-T Extinction",
    "3925": "Ventura Robotics",
    "3926": "MPArors",
    "3928": "Team Neutrino",
    "3931": "Cockadoodle Dominators",
    "3932": "The Dirty Mechanics",
    "3933": "PrepaTec - Taman Keet",
    "3936": "RoboBlitz 3936",
    "3937": "Breakaway",
    "3939": "Botetourt 4-H Robotics",
    "3940": "CyberTooth",
    "3944": "All Knights",
    "3950": "RoboGym Robotics",
    "3951": "SUITS",
    "3952": "Troy Robotics",
    "3954": "4-H Electrotechs",
    "3958": "Schrodinger's Cat",
    "3959": "Mech Tech",
    "3965": "Sultan Robotics Team 3965",
    "3966": "Gryphon Command",
    "3970": "Duncan Dynamics",
    "3975": "Les Dragons",
    "3976": "Electric Hornets",
    "3984": "Topper Robotics",
    "3985": "Sonic Howl",
    "3986": "Express-O",
    "3990": "Tech for Kids",
    "3996": "RIKITIK",
    "4003": "TriSonics",
    "4004": "M.A.R.S. Rovers",
    "4005": "Hostile Gato Robotics",
    "4009": "DNA Robotics",
    "4010": "PrepaTec - Nautilus",
    "4013": "Clockwork Mania",
    "4015": "Jags",
    "4019": "Mechanical Paradise",
    "4020": "Cyber Tribe",
    "4021": "igKnightion",
    "4024": "TechnoSaints",
    "4026": "Decatur Robotics",
    "4027": "Centre Punch",
    "4028": "The Beak Squad",
    "4034": "Pelham πthons Team 4034",
    "4039": "MakeShift Robotics",
    "4041": "Iron Tigers",
    "4043": "NerdHerd",
    "4048": "Redshift",
    "4050": "Biohazard",
    "4054": "The Aeronauts",
    "4060": "Bearcat Robotics",
    "4061": "SciBorgs",
    "4063": "TriKzR4Kidz",
    "4065": "Nerds of Prey",
    "4068": "Bearbotics",
    "4069": "Lo-Ellen Robotics",
    "4076": "Texan Robotics",
    "4079": "Quantum Leap",
    "4083": "The Legendary Iron Wolverines",
    "4085": "Technical Difficulties",
    "4087": "Voodoo Voltage",
    "4089": "Stealth Robotics",
    "4091": "DRIFT",
    "4093": "Hardwired",
    "4096": "Ctrl-Z",
    "4099": "The Falcons",
    "4107": "Team Storm",
    "4112": "EagleBots",
    "4118": "Roaring Riptide",
    "4121": "Viking Robotics",
    "4122": "Ossining OBOTS",
    "4123": "Tribe Robotics",
    "4125": "Confidential",
    "4130": "The Blue Devils",
    "4131": "Iron Patriots",
    "4135": "Iron Patriots",
    "4141": "Monarch Robotics",
    "4143": "MARS/WARS",
    "4145": "WorBots",
    "4146": "Sabercats",
    "4150": "FRobotics",
    "4152": "Hoya Robotics",
    "4153": "Project Y",
    "4156": "Ninjaneers",
    "4159": "CardinalBotics",
    "4160": "The RoBucs",
    "4166": "Robostang",
    "4169": "Warrior Robotics",
    "4171": "BayBots",
    "4173": "Bark Bots",
    "4174": "Mustang Robotics",
    "4175": "Coded Summit",
    "4176": "Iron Tigers",
    "4179": "Tahisco Techs",
    "4180": "Iron Riders",
    "4181": "Quack Attack",
    "4182": "Viking Robotics",
    "4183": "Bit Buckets",
    "4186": "Alameda Aztechs Robotics",
    "4188": "Columbus Space Program",
    "4189": "Chargers",
    "4191": "IMC",
    "4192": "Jaguar Robotics",
    "4198": "RoboCats",
    "4201": "The Vitruvian Bots",
    "4206": "Robo Vikes",
    "4207": "PyroBotics",
    "4213": "MetalCow Robotics",
    "4215": "TRIBOTS",
    "4216": "Blue Ops Robotics",
    "4219": "RED!",
    "4225": "RoboCats",
    "4226": "Huskies",
    "4229": "Magnetech",
    "4230": "TopperBots",
    "4237": "Team Lance-A-Bot",
    "4238": "Resistance Robotics",
    "4239": "WARPSPEED",
    "4240": "TroTek Warriors",
    "4253": "Raid Zero",
    "4255": "Robodores",
    "4256": "Cyborg Cats",
    "4260": "BEAR (Blue Earth Area Robotics)",
    "4265": "Secret City Wildbots",
    "4269": "CardinalBots",
    "4270": "Crusaders",
    "4272": "Maverick Robotics",
    "4276": "Surf City Vikings",
    "4277": "Thingamajiggers",
    "4285": "Camo-Bots",
    "4286": "Imperial Robotics",
    "4293": "Komodo",
    "4295": "Hudson Stingers",
    "4296": "Trident Robotics",
    "4299": "BCS RoboSharks",
    "4306": "Robokomodos",
    "4308": "ABSOLUTE ROBOTICS",
    "4311": "Swampscott Currents",
    "4314": "Springfield Urban League Abe-Ro-Bots",
    "4319": "Ladies FIRST",
    "4320": "The Joker",
    "4322": "Clockwork",
    "4325": "RoboRangers",
    "4327": "Q Branch",
    "4328": "Furious Falcons",
    "4329": "Lutheran Roboteers",
    "4330": "Rambunction",
    "4332": "RoboCats",
    "4336": "Ramageddon Robotics",
    "4338": "Falcons",
    "4342": "Demon Robotics",
    "4355": "Samurai Code - Peñoles - John Deere",
    "4360": "Spudnik",
    "4361": "Roxbotix",
    "4362": "Gems",
    "4364": "Metal Jackets",
    "4371": "Prepa Tecmilenio - ART",
    "4375": "Westend Gearbusters",
    "4377": "Boyne City Blaze",
    "4381": "Twisted Devils",
    "4383": "The P-TECH Fly-Bots",
    "4384": "Benzene Bots",
    "4388": "Ridgebotics",
    "4390": "ATA Coregears",
    "4391": "BraveBots",
    "4392": "The Deceivers",
    "4395": "Dundee Vi-Borgs",
    "4398": "Gladiators",
    "4400": "Cerbotics - Peñoles",
    "4401": "CETMOV - Peñoles",
    "4403": "PrepaTec - ROULT - Peñoles",
    "4405": "The Atoms Family",
    "4414": "HighTide",
    "4415": "EPIC Robotz",
    "4416": "Skynet",
    "4418": "IMPULSE",
    "4419": "Team Rewind",
    "4421": "FORGE Robotics",
    "4422": "Twisted Angels",
    "4423": "MARS' Minions",
    "4450": "Olympia Robotics Federation",
    "4451": "ROBOTZ Garage",
    "4453": "The Red Hot Chili Bots",
    "4455": "The Burger Bots",
    "4456": "Mech Cadets",
    "4458": "Infernobotix",
    "4459": "LeoTechs",
    "4464": "Team Illusion",
    "4467": "Titanium Titans",
    "4468": "Fernbank LINKS",
    "4469": "R.A.I.D. (Raider Artificial Intelligence Division)",
    "4470": "TiGears",
    "4472": "SuperNOVA",
    "4473": "Delta Prime Robotics 4-H",
    "4476": "W.A.F.F.L.E.S.",
    "4480": "UC-Botics",
    "4481": "Team Rembrandts",
    "4485": "Tribe Tech Robotics",
    "4488": "Shockwave",
    "4499": "The Highlanders",
    "4500": "RoboHounds",
    "4501": "Humans",
    "4504": "B. C. Robotics",
    "4505": "McDonogh Robotics",
    "4509": "Mechanical Bulls",
    "4511": "Power Amplified",
    "4512": "Otter Chaos",
    "4513": "Circuit Breakers",
    "4522": "Team SCREAM",
    "4523": "MCROBO",
    "4531": "STEAMpunk",
    "4533": "Phoenix",
    "4534": "Wired Wizards",
    "4536": "MinuteBots",
    "4539": "KAOTIC Robotics",
    "4541": "CAVineers",
    "4546": "Shockwave",
    "4550": "Something's Bruin",
    "4561": "TerrorBytes",
    "4564": "Orange Chaos",
    "4567": "The Mechanized Mafia",
    "4568": "Robot Commander",
    "4571": "Rambots",
    "4572": "Barlow Robotics",
    "4573": "Rambotics",
    "4575": "Gemini",
    "4576": "Red Nation Robotics",
    "4579": "RoboEagles",
    "4580": "Conductors",
    "4584": "SentinelFox Engineering",
    "4585": "Husky Robotics",
    "4586": "PRIMO",
    "4587": "Jersey Voltage",
    "4590": "GreenBlitz",
    "4593": "Rapid Acceleration",
    "4597": "SABERBOTS",
    "4598": "ICARUS INNOVATED",
    "4601": "Circuit Birds",
    "4603": "Forza One - Peñoles",
    "4607": "C.I.S.",
    "4610": "BearTecs",
    "4611": "OZone",
    "4613": "Barker Redbacks",
    "4617": "DAUN",
    "4619": "HawkBots",
    "4622": "CAT 5",
    "4623": "Flyer Robotics",
    "4624": "Rebel Alliance",
    "4627": "Manning Robotics",
    "4628": "Sintian Robotics",
    "4630": "Robodragons",
    "4632": "Monti-Pythons",
    "4635": "PrepaTec - Botbusters",
    "4637": "BambieBotz",
    "4638": "Jagbots",
    "4639": "The RoboSpartans",
    "4641": "TALON Inc.",
    "4643": "Butte Built Bots",
    "4645": "The Chicago Style BotDogs",
    "4646": "Wildcard Robotics",
    "4652": "Ironmen 2",
    "4653": "Ironmen Robotics",
    "4655": "Stateline Robotics",
    "4656": "Rock Solid Robotics",
    "4661": "Cypher",
    "4662": "Byte Sized Robotics",
    "4663": "Cyber Tigers",
    "4664": "Butler Bots",
    "4669": "Galileo Robotics",
    "4674": "RoboJacks",
    "4678": "CyberCavs",
    "4680": "AzTech Eagles",
    "4682": "CyBears",
    "4687": "Spartobots",
    "4688": "Saints Bot",
    "4692": "Metal Mallards",
    "4693": "Talk Nerdy to Me",
    "4698": "Raider Robotics",
    "4701": "Warriors (Team W.I.R.E.)",
    "4702": "Cyberdogz",
    "4707": "FORCE",
    "4711": "The Flying Aces",
    "4717": "Westerner Robotics",
    "4723": "Prepa Tecmilenio - EARTH - 4723",
    "4728": "ROCORI Robotics",
    "4729": "EMU Robotics",
    "4730": "Terminators",
    "4731": "MONARCH-E",
    "4734": "The Iron Plainsmen",
    "4735": "Prepa Tecmilenio - DEROF - Peñoles",
    "4737": "Atomigators",
    "4738": "Patribots",
    "4739": "Normanhurst Nitro",
    "4741": "WingNuts",
    "4744": "Ninjas",
    "4750": "Bert",
    "4758": "RoboBlazers",
    "4761": "The Robockets",
    "4764": "The Memphis MechWarriors",
    "4765": "PWRUP",
    "4766": "Team SCREAM Jr.",
    "4768": "Cody \"The Green Comet\"",
    "4774": "The Drop Bears",
    "4775": "Aztech Robotics",
    "4776": "S.C.O.T.S. Bots",
    "4778": "Stormbots",
    "4779": "RoboSapiens",
    "4782": "PrepaTec - Borrebots",
    "4786": "Nicolet FEAR",
    "4787": "Axiom",
    "4788": "Can't Control",
    "4795": "Eastbots",
    "4809": "Black Knight Robotics",
    "4810": "I AM Robot",
    "4816": "Gadget Girls",
    "4817": "One Degree North",
    "4821": "CyberUs",
    "4827": "Minerbotics",
    "4828": "RoboEagles",
    "4829": "Titanium Tigers",
    "4835": "Junkyard Dawgs",
    "4838": "Under Currents",
    "4851": "JungleBots",
    "4854": "Trobots",
    "4855": "Ramageddon",
    "4859": "The CyBears",
    "4861": "Beastbots",
    "4862": "Manning Mountaineers",
    "4863": "The Chicago Style BotPups",
    "4864": "Innovation Station",
    "4904": "Bot-Provoking",
    "4905": "Andromeda One",
    "4907": "Thunderstamps",
    "4909": "Bionics",
    "4911": "CyberKnights",
    "4915": "Spartronics",
    "4917": "Sir Lancerbot",
    "4919": "Team Ronin",
    "4920": "Belle River Boltheads",
    "4925": "The Resistance",
    "4926": "GalacTech",
    "4930": "Electric Mayhem",
    "4931": "Edwardsville Tech",
    "4935": "T-Rex",
    "4940": "Knight Vision",
    "4944": "The Hi Fives",
    "4946": "The Alpha Dogs",
    "4947": "Fenix",
    "4951": "CDS Cyclones",
    "4952": "Les Carnicas",
    "4955": "LA Tech",
    "4956": "RoboSharks",
    "4959": "Staley Robotics",
    "4961": "Shock and Awe-sum",
    "4964": "Roosevelt Robotics",
    "4967": "That ONE Team-OurNextEngineers",
    "4972": "BORUSAN ROBOTICS",
    "4973": "Gator Gears",
    "4976": "Revolt Robotics",
    "4977": "Iron Lion",
    "4980": "Canine Crusaders",
    "4982": "Olympus Robotics",
    "4983": "IronBots",
    "4984": "Bullseye",
    "4990": "Basement Bots",
    "4991": "Horsepower",
    "4994": "Kingston Robo-Cards",
    "4998": "TESLA",
    "4999": "Momentum",
    "5000": "HAMMERHEADS",
    "5002": "Dragon Robotics",
    "5010": "Tiger Dynasty",
    "5012": "Gryffingear",
    "5013": "Trobots",
    "5016": "Huntington Robotics",
    "5019": "Gearbox Heroes",
    "5024": "Raider Robotics",
    "5025": "Pacific Steel",
    "5026": "Iron Panthers",
    "5027": "Event Horizon (5027)",
    "5031": "Full Metal Mustangs",
    "5032": "The Falcons",
    "5036": "The Robo Devils",
    "5041": "CyBears",
    "5045": "SpartaBot",
    "5046": "Jacked Up Jackets",
    "5049": "Minerbots",
    "5050": "Cow Town Robotics",
    "5053": "Lakers",
    "5060": "IgKnight",
    "5065": "Knights #5065",
    "5066": "Singularity",
    "5067": "Steiner Steel Storm",
    "5070": "Gearnotics",
    "5071": "Charger Robotics",
    "5074": "RoboMustangs",
    "5084": "Team FridgeBot",
    "5086": "Cadillac Connectors",
    "5089": "Robo-Nerds",
    "5090": "TorqueNados",
    "5096": "The Teutonic Force",
    "5098": "STING - R",
    "5103": "MECH&Cheese",
    "5104": "BreakerBots",
    "5109": "Gladiator Robotics",
    "5110": "Roboherd",
    "5112": "The Gongoliers",
    "5113": "Combustible Lemons",
    "5114": "Titanium Tigers",
    "5115": "Knight Riders",
    "5119": "Team STEAM",
    "5123": "Mechadogs",
    "5124": "West Torrance Robotics",
    "5125": "Hawks on the Horizon",
    "5126": "Electromagnetic Panthers (EMP)",
    "5133": "PrepaTec - Blue Steel",
    "5135": "Black Unicorns",
    "5136": "MechaPirates",
    "5137": "Iron Kodiaks",
    "5141": "GRIFFINITE",
    "5142": "RoboDominators",
    "5143": "Raw Steel",
    "5144": "LISD TECHnicians",
    "5145": "WolfBotics",
    "5147": "Hornets",
    "5148": "New Berlin Blitz",
    "5149": "TroyBotics",
    "5150": "Hybrid Hornets",
    "5152": "Alotobots",
    "5160": "Chargers",
    "5162": "The Big Red Theory",
    "5166": "Fabricators",
    "5167": "Vi-Bots",
    "5171": "Deus Ex Machina",
    "5176": "RoboBills",
    "5181": "Explorer Robotics",
    "5182": "Chieftainators",
    "5183": "Devil Bots",
    "5188": "Area 5188: Classified Robotics",
    "5189": "Team K.I.S.S.",
    "5190": "Falcon Fury",
    "5193": "Pantheon",
    "5197": "Northwestern High",
    "5199": "Robot Dolphins From Outer Space",
    "5205": "FullMetal-Jackets",
    "5212": "TAMSformers Robotics",
    "5213": "St. Ignace SHIELD",
    "5214": "The Mighty CavBots",
    "5216": "E-Ville Empire",
    "5219": "TeknoSquad 5219",
    "5225": "The Shambotz",
    "5227": "Eagle Elite",
    "5229": "Heritage Hawkbots",
    "5231": "The Radical Jays",
    "5232": "Talons",
    "5234": "MarauderBots",
    "5235": "Gears of Fortune",
    "5239": "The Royal Stags",
    "5243": "Aegis",
    "5247": "Red Devil Robotics",
    "5248": "Robo-Saxons",
    "5253": "Bigfork Backwoods Bots 5253",
    "5256": "Atomics",
    "5260": "Owosso Robotics",
    "5261": "Cyber Shock",
    "5263": "Robo-Bucs",
    "5268": "The BioMech Falcons",
    "5271": "Open Circuits",
    "5274": "Wolverine Robotics",
    "5275": "T.I.M.E.-Bots",
    "5276": "Edgar Allan Ohms",
    "5278": "Los Clasicos",
    "5282": "Railroader Robotics",
    "5285": "Sea Kings Robotics",
    "5291": "Emperius",
    "5293": "Metal Crusaders",
    "5295": "Aldernating Current",
    "5298": "E-TECH CHARGERS",
    "5312": "Lobotics - Peñoles",
    "5314": "Ice Bots",
    "5331": "Lightning Bots",
    "5338": "RoboLoCo",
    "5339": "Hurricanes",
    "5347": "Gryphons",
    "5348": "Charger Robotics",
    "5353": "LoCo Raptors",
    "5401": "Fightin' Robotic Owls",
    "5402": "Wreckless Robotics",
    "5406": "Celt-X",
    "5407": "Wolfpack Robotics",
    "5408": "Kennedy Kinetics",
    "5409": "Chargers",
    "5410": "Eaglebotics",
    "5411": "RoboTalons",
    "5413": "Stellar Robotics",
    "5414": "Pearadox",
    "5417": "Eagle Robotics",
    "5419": "Berkelium",
    "5421": "BirchBots",
    "5422": "Stormgears",
    "5424": "Rogue Robots",
    "5427": "Steel Talons",
    "5430": "Pirate Robolution",
    "5431": "Titan Robotics",
    "5434": "Falcon Robotics",
    "5436": "Cyber Cats",
    "5437": "Rocky Balboabots",
    "5438": "Technological Terrors",
    "5439": "Huskies",
    "5442": "Mechanical Monarchy",
    "5443": "Les PATenteux",
    "5449": "Prototype",
    "5454": "Obsidian",
    "5458": "Digital Minds",
    "5459": "Ipswich TIGERS",
    "5460": "Strike Zone",
    "5461": "Victorian Engineered Robotic Nation or V.E.R.N.",
    "5462": "2Paw Robotics",
    "5464": "Bluejacket Robotics",
    "5467": "RoboTractors",
    "5468": "Chaos Theory",
    "5470": "The Cowpunchers",
    "5472": "The Wyld Stallyns",
    "5474": "Clairemonster",
    "5478": "Pershing Doughbots",
    "5484": "Career Academy Robotics - Wolf Pack",
    "5486": "Robotic Turmoil",
    "5490": "The Dark Byte",
    "5491": "Hard Reset",
    "5492": "Winner's Circle Robo Jockey's",
    "5493": "SMAbotics",
    "5494": "Bizarbots Robotics",
    "5498": "The Wired Devils",
    "5499": "Bearium",
    "5500": "Jaguars Robotics & ROVs",
    "5501": "Bobcats",
    "5503": "Smithville Tiger Trons",
    "5504": "The Loose Connections",
    "5505": "V2 - Volts Squared",
    "5507": "Robotic Eagles",
    "5509": "Like a Boss",
    "5511": "Cortechs Robotics",
    "5514": "MavBots",
    "5515": "Blue Power Robotics",
    "5516": "Iron Maple",
    "5517": "The Engineers on Fire",
    "5518": "Techno Wolves",
    "5522": "Stargazer",
    "5525": "Alcona Tool Cats",
    "5526": "Type C - Peñoles",
    "5527": "Cyber Phoenix",
    "5528": "Ultime",
    "5530": "Greenhills Lawnmowers",
    "5531": "Orange Crush",
    "5532": "The Panther Bots",
    "5533": "Electric Horse Power",
    "5534": "Onaway Onabots",
    "5535": "Bionic Bison",
    "5538": "Vikingbots",
    "5541": "Shakopee Sabers",
    "5547": "The Talons",
    "5549": "Gryphon Robotics",
    "5550": "BRONChO BOTS",
    "5553": "Robo'Lyon",
    "5554": "The Poros Robotics",
    "5557": "BB-R8ERS",
    "5559": "Gear Grinders",
    "5560": "Central Lake Trobotics",
    "5561": "Raider Robotics",
    "5563": "Phalanx",
    "5567": "Code Red Robotics",
    "5572": "Rosbots",
    "5576": "Team Terminator",
    "5577": "Kinematic Wolves",
    "5583": "SistamatiK",
    "5584": "ICRobotics",
    "5586": "Bond Brigade",
    "5587": "Titan Robotics",
    "5588": "Reign Robotics",
    "5590": "Alumiboti",
    "5596": "Wolverine Robotics",
    "5603": "The WarriorBots",
    "5607": "Team Firewall",
    "5608": "Lassiter Robotics",
    "5610": "Turbulence",
    "5612": "Robo-Raptors",
    "5613": "ThunderDogs",
    "5614": "Team Sycamore",
    "5618": "PLS",
    "5623": "Robotic Rams",
    "5624": "TIGER TECH Robotics",
    "5635": "Demacia",
    "5638": "JukeBots",
    "5641": "Byron Robotics",
    "5642": "Mumford Mechanical Mustangs",
    "5648": "Melbourne RoboCats 5648",
    "5651": "Maynard Jackson RoboJags",
    "5653": "Iron Mosquitos",
    "5654": "Phoenix",
    "5655": "KelRot",
    "5658": "STORMBOTICS",
    "5660": "Symmetrical Chaos",
    "5665": "FENERBAHÇE DOGUS SPARC",
    "5667": "The Digital Eagles",
    "5669": "Techmen",
    "5672": "First Nations-STEM",
    "5674": "The Gearhounds",
    "5675": "WiredCats",
    "5676": "The H.E.R.O.E.S",
    "5682": "Equus Engineering",
    "5683": "R.A.V.E.N.",
    "5684": "Titans of Tech - Thrive Charter School",
    "5687": "The Outliers",
    "5689": "CK Cyber Pack",
    "5690": "SubZero Robotics",
    "5695": "Heavy Duty Eagles",
    "5697": "Bearcats",
    "5700": "SOTA Cyberdragons",
    "5702": "Robotowatomi",
    "5704": "Weird & Wired",
    "5705": "DESERT EAGLES - JOHN DEERE",
    "5706": "Tahquamenon Phenomenon",
    "5708": "Zebrotics",
    "5712": "Hemlock's Gray Matter",
    "5715": "DRC",
    "5716": "PrepaTec - Keybot",
    "5719": "Pink Titans",
    "5720": "Jagobotics",
    "5722": "Plutonium Panthers 5722",
    "5724": "Spartan Robotics",
    "5726": "RHUMBOTZ",
    "5727": "Omegabytes",
    "5728": "MC2",
    "5732": "ROBOTIGERS",
    "5735": "Control Freaks",
    "5736": "Kingsmen Robotics",
    "5740": "Trojanators",
    "5744": "RoboRunners",
    "5746": "Human Error",
    "5756": "REC'in Crew",
    "5760": "The Marinators",
    "5773": "YAFL Mechatronics",
    "5788": "Eagle Spec Ops",
    "5789": "The Other Owls",
    "5790": "Proto-Titans",
    "5800": "Magic Island Robotics",
    "5801": "CTC Inspire",
    "5804": "TORCH",
    "5805": "SMbly Required",
    "5806": "Basement Lions",
    "5809": "The Jesubots 5809",
    "5811": "BONDS",
    "5813": "Morpheus",
    "5816": "Gra-V Robotics",
    "5817": "Uni-Rex",
    "5822": "WolfByte",
    "5823": "ACE",
    "5826": "Avis Automata",
    "5828": "Flying Riots",
    "5829": "Awtybots",
    "5830": "LIFE Engineering",
    "5835": "Sage Hill Robotics",
    "5837": "Unity4Tech",
    "5841": "The Patriots",
    "5842": "Royal Robotics",
    "5843": "Flurb",
    "5847": "Ironclad",
    "5849": "Joker",
    "5851": "Striking Vikings",
    "5856": "Bullbots",
    "5857": "Walnut Valley Robotics",
    "5860": "Full Metal Muskrats",
    "5863": "Hypercubed",
    "5866": "Fe [Iron] Tigers",
    "5869": "QHHS Aqueducks",
    "5870": "League of Logic",
    "5871": "Chickadees",
    "5872": "Wiredcats",
    "5874": "Prepa Tecmilenio BorderBots",
    "5876": "ARTEMIS",
    "5878": "The Great Lakers",
    "5881": "TVHS Dragons",
    "5883": "Spice Gears",
    "5885": "Villanova WiredCats",
    "5887": "PrepaTec IMPERATOR",
    "5889": "Commandobots",
    "5892": "Energy HEROs",
    "5894": "CyberJags",
    "5895": "Peddie Robotics",
    "5900": "The Fighting Mongooses",
    "5901": "Electric Eels",
    "5902": "The Wire Clippers",
    "5903": "ThorBots",
    "5907": "CC Shambots",
    "5908": "Spartans",
    "5912": "Copper Hawks",
    "5913": "Patriotics",
    "5914": "Robotic Warriors",
    "5918": "Pumatrons",
    "5919": "JoCo RoBos",
    "5920": "VIKotics",
    "5923": "WildC.A.T.S.",
    "5924": "Golden Gate Robotics",
    "5926": "Da Moose",
    "5927": "Globetrotters",
    "5928": "MetalBoost",
    "5929": "Raider Robotics",
    "5932": "Prepa Tecmilenio - Quetzales",
    "5933": "JudgeMent Call Robotics",
    "5934": "Crowbotics",
    "5935": "Tech Tigers",
    "5937": "MI Robotics",
    "5940": "BREAD",
    "5941": "Cast Iron Orcas",
    "5948": "PrepaTec - Lebotics",
    "5951": "Makers Assemble",
    "5959": "PrepaTec - Titanium Rams",
    "5962": "perSEVERE",
    "5965": "Power Struck Girls",
    "5968": "Little Apple Cyborgs",
    "5970": "Beavertronics",
    "5975": "Beta Blues",
    "5980": "East Grand Rapids Robotics",
    "5982": "Merrill's Circuits of Destruction",
    "5985": "Project Bucephalus",
    "5986": "┌ Iron Fangs ┐",
    "5987": "Galaxia in memory of David Zohar",
    "5988": "Narooma High RoboRebels",
    "5990": "TRIGON",
    "5992": "Pirates",
    "5993": "Istech Robotics Society",
    "5996": "R.U.R.",
    "6000": "Firehawk Robotics",
    "6002": "ZooBOTix",
    "6004": "f(x) Robotics",
    "6005": "Cardinaltronics",
    "6014": "ARC",
    "6016": "Tiger Robotics",
    "6017": "PrepaTec - Cyberius",
    "6023": "DISCBOTS",
    "6024": "R Factor",
    "6025": "Adroit Androids",
    "6026": "Tiger Strike",
    "6029": "Railbots",
    "6032": "Pirate Robotics",
    "6033": "Gadget Agents",
    "6036": "Peninsula Robotics",
    "6038": "ITOBOT",
    "6044": "Circuit Breakers",
    "6045": "Sabre Robotics",
    "6047": "Proctor Frostbyte",
    "6057": "Thunderbots",
    "6059": "System Overload Robotics",
    "6060": "Circuit Serpents",
    "6064": "IstanBULLS",
    "6067": "The Underdogs",
    "6068": "Delta Overload",
    "6071": "The Aluminators",
    "6072": "Triton Tech 6072",
    "6073": "Cav-Bots",
    "6077": "Wiking Kujon",
    "6078": "RoboRams",
    "6079": "JET SET",
    "6081": "Digital Dislocators",
    "6083": "Overlooking",
    "6085": "Green Devil Bots",
    "6087": "Cybertronic Lancers",
    "6088": "Stephenson Eagles",
    "6090": "Wayland Wildcats",
    "6091": "Eagle 1",
    "6094": "Manic Mechanics",
    "6096": "UPSM Cobrots",
    "6098": "bIrobot",
    "6099": "Knight Riders",
    "6100": "MS Mobots",
    "6101": "Strange Quarks",
    "6104": "Desert Eagles in memory of Yehonatan Maimon",
    "6106": "PrepaTec - TecGear",
    "6107": "CyberJagzz",
    "6110": "Doc Botics",
    "6112": "Cyberstangs",
    "6113": "M.A.R.C. 1",
    "6114": "Hamilton CyberHawks",
    "6120": "CyberStangs",
    "6121": "RoboVikes",
    "6122": "Potential Energy",
    "6128": "M.O.T.O.R (More Than Our Robot)",
    "6132": "Iron Rangers",
    "6135": "Arctos",
    "6145": "War Eagles",
    "6146": "Blackjacks",
    "6147": "Tonkabots",
    "6152": "Robo-Falcons",
    "6153": "Blue Crew",
    "6155": "ElektraBots",
    "6162": "Cap Alpaca 6162",
    "6166": "Thorobotics",
    "6167": "CB Bearbots",
    "6168": "alzahrawi",
    "6170": "Prepa Tecmilenio Vitronik",
    "6171": "⚛ Chain Reaction ⚛",
    "6175": "EVW Mystery Machine",
    "6180": "Mechanical Lions",
    "6184": "C4 (Capital City Chaotic Coders)",
    "6186": "Hope BotCats",
    "6188": "Sting Robotics",
    "6190": "Cybernetic Crusaders",
    "6191": "RoboKryptonite",
    "6199": "E-Hawks - Peñoles",
    "6200": "PrepaTec - XRAMS",
    "6201": "The Highlanders",
    "6217": "Bomb-Botz",
    "6220": "The Golden Griffins",
    "6223": "Arsenal of Engineering",
    "6226": "Blue Devils",
    "6228": "MAT Robotics",
    "6230": "Team Koi",
    "6232": "Florya Bisons",
    "6237": "Martin Motion",
    "6238": "Popcorn Penguins",
    "6239": "The Irrational Engineers",
    "6240": "Eagles of the Knight",
    "6242": "Chaos Theory",
    "6243": "Energy NERDs",
    "6244": "Light SaBres",
    "6300": "The Northwoodians",
    "6302": "Greeneville High School Robotics",
    "6304": "EAGLE",
    "6305": "Stable Circuits",
    "6318": "FE Freedom Engineers",
    "6322": "OneClay ARMORY",
    "6324": "The Blue Devils",
    "6326": "⚡ Baltimore Bolts ⚡",
    "6328": "Mechanical Advantage",
    "6329": "The Bucks' Wrath",
    "6333": "Scavenger Robotics",
    "6340": "The Marist Manatees",
    "6343": "Steel Ridge",
    "6344": "Yale Jiggawattz",
    "6346": "Cybears Robotics Team",
    "6348": "PrepaTec - HORUS",
    "6350": "Clawbots",
    "6352": "LAUNCH TEAM",
    "6353": "EFZ Robotics",
    "6357": "The Spring Konstant",
    "6358": "The Buhlean Operators",
    "6364": "Ten Ton",
    "6366": "RAM Rodz Robotics",
    "6367": "The ElectroLights",
    "6369": "Mercenary Robotics",
    "6377": "Howdy Bots",
    "6381": "Red Raider Robotics",
    "6390": "Hephaestus",
    "6391": "Ursuline Bearbotics",
    "6394": "MITO",
    "6397": "Romero Robotics",
    "6399": "Tinspiratio",
    "6401": "8-bit RAMs",
    "6402": "GÖKTÜRKLER",
    "6404": "Brazilian Storm",
    "6410": "Supreme Eagles",
    "6413": "Degrees of Freedom",
    "6414": "Voyager",
    "6415": "Gultepe Robotics Team 6415",
    "6417": "Fridolins",
    "6418": "The Missfits",
    "6419": "Kernel Chaos",
    "6420": "Fire Island Robotics",
    "6421": "WarriorBots",
    "6423": "Ward Melville Iron Patriots",
    "6424": "Stealth Panther Robotics",
    "6428": "STEMINEERS",
    "6429": "4th Dimension",
    "6430": "Kalsedon",
    "6431": "NoktaParantez",
    "6432": "Valkyries",
    "6433": "HZ4Z",
    "6434": "Bossley Park High School Wolfgang",
    "6436": "PARS ROBOTICS",
    "6443": "AEMBOT",
    "6453": "Bog Bots!",
    "6455": "The Coded Collective",
    "6459": "AG Robotik",
    "6463": "Robotic Reapers",
    "6465": "Mystic Biscuit",
    "6473": "Rock Star Robotics",
    "6476": "Supernova Star Squad",
    "6479": "AZTECH Robotics",
    "6483": "Prepa Tecmilenio - CHARGER",
    "6487": "Clockwork Knights",
    "6488": "RoboRams",
    "6489": "Bronco Bolts",
    "6494": "Wings of Liberty",
    "6497": "Pymble P-Platers",
    "6498": "Castle Robotics",
    "6500": "GearCats",
    "6502": "DARC SIDE",
    "6506": "Steel Boot Robotics",
    "6508": "Hastings Heroes",
    "6510": "Pymble Pride",
    "6515": "Unimatics",
    "6517": "So-Kno Robo",
    "6526": "Cyber Rangers",
    "6527": "RamShock",
    "6528": "Terror Bytes",
    "6532": "TechnoPaths",
    "6533": "BeaverTech noducks",
    "6538": "Linc-Bots",
    "6548": "Perry RAMBOTS",
    "6556": "Spartan Robotics 6556",
    "6558": "Aluminum Sting",
    "6560": "Charging Champions",
    "6566": "Battering Rams",
    "6569": "Gladiators",
    "6570": "Whiteford Bobcats",
    "6574": "Ferradermis",
    "6588": "Counter-Torque Robotics",
    "6590": "SaviTech",
    "6591": "Panther Powered Robotics 6591",
    "6593": "HyperDrive Robotics",
    "6606": "Prepa Tecmilenio Pink Hawks 6606",
    "6608": "Prepa Tecmilenio - ARCADIA",
    "6610": "Robot Roll Call",
    "6615": "Bellevillains",
    "6616": "Mechanical Pumas",
    "6618": "WARHBOTS",
    "6619": "GravitechX",
    "6620": "Voltage Vikings",
    "6621": "Artemis",
    "6622": "Stan Robotix",
    "6628": "KMS BOTKICKERS",
    "6632": "Northview Robotics",
    "6637": "BetaWolves",
    "6639": "The Mechanical Minds",
    "6642": "Harrison Stingers of Steel",
    "6643": "Walnuts and Bolts",
    "6645": "Alloy Obsession",
    "6647": "PrepaTec - VOLTEC",
    "6648": "MechaSpartans",
    "6651": "Golden Bots",
    "6652": "Tigres",
    "6657": "Arborbotics",
    "6658": "Rockbotics",
    "6662": "FalconX",
    "6665": "C-Otters",
    "6672": "Fusion Corps",
    "6676": "Prepa Tecmilenio - Thunder Hawks - 6676",
    "6686": "Bionic Broncos",
    "6690": "MV roboPride",
    "6691": "Torque",
    "6695": "GalvaKnights",
    "6696": "Cardinal Dynamics",
    "6702": "PrepaTec - StingBots",
    "6704": "Mid Pacific Owl Robotics",
    "6705": "Wildcat5e",
    "6706": "Golem",
    "6709": "S.P.U.D. INC",
    "6712": "Mountaineers",
    "6721": "Tindley Trailblazers",
    "6722": "LED Robotics",
    "6725": "WildBOTS",
    "6729": "RobCoBots",
    "6731": "Record Robotics",
    "6732": "BHS RoboRaiders",
    "6738": "Excalibur",
    "6740": "G3 - Glue Gun & Glitter",
    "6743": "Tiger Engineering",
    "6744": "MEHLVILLE RC",
    "6749": "tERAbytes",
    "6753": "CBA RoboKings",
    "6758": "Otternauts",
    "6762": "Oscats",
    "6764": "Flash Drives",
    "6766": "AtomStorm",
    "6768": "Denison Robo-Jackets",
    "6772": "The Marist Manta Rays",
    "6773": "Max Throttle Mavericks",
    "6774": "Oh-Kno Robo",
    "6800": "Valor",
    "6802": "Snot Otters",
    "6805": "Quack Attack",
    "6806": "Mineola Wild Reds",
    "6808": "William Tennent Robotics",
    "6809": "Aviators",
    "6814": "Ellipse",
    "6817": "10 Factorial",
    "6822": "THE SHISHKABOTS",
    "6824": "LVA Robot Pigeons",
    "6829": "Ignite Robotics",
    "6831": "A-05 Annex",
    "6832": "STEAMex",
    "6833": "Python Robotics",
    "6834": "Bionic Warriors",
    "6838": "X-SHARC",
    "6843": "Guardian Gears",
    "6845": "River Bots",
    "6854": "Viking Robotics",
    "6859": "Big Metal Lakers 6859- BML Robotics",
    "6860": "Equitum Robotics",
    "6861": "The Tyros Team",
    "6863": "Squirrels",
    "6864": "GryphTech Robotics",
    "6865": "Manitoulin Metal",
    "6868": "Cadet Robotics",
    "6869": "Gladiateurs",
    "6872": "Panthera Ferro",
    "6873": "The Generals",
    "6874": "Mediterra",
    "6875": "Amazon Warriors Robotics",
    "6877": "Mechanical Mustangs",
    "6880": "Universal Serial Brawlers (USB)",
    "6882": "Fahrenheit Robotics",
    "6884": "Deep-Space Robotics",
    "6885": "The Pilots",
    "6887": "Dalton Catabots",
    "6888": "Foothills Robotics",
    "6894": "Iced Java",
    "6895": "Mechanical Madness 4-H",
    "6896": "CABOTS - Fresnillo PLC - Conalep Caborca",
    "6897": "Astraea Robotics",
    "6901": "Knights Robotics",
    "6902": "STRIKE",
    "6904": "TeraWatts",
    "6905": "Raiders of the ARC- TEAR-A-BYTE",
    "6907": "The G.O.A.T",
    "6908": "Infuzed",
    "6909": "SAKURA Tempesta",
    "6910": "Mill Creek Steel Talons",
    "6911": "SO BOTZ",
    "6914": "Retro5ive",
    "6916": "Iron Thunder",
    "6918": "Napa Cellar Rats",
    "6919": "The Commodores",
    "6921": "Technados",
    "6925": "W.A.Robotics",
    "6929": "Cuivre & Or",
    "6932": "SMART",
    "6933": "Archytas",
    "6934": "Scorps",
    "6936": "Vault 6936",
    "6940": "Violet Z",
    "6941": "IronPulse Robotics",
    "6945": "Children of the Corn",
    "6947": "Savage Tumaz",
    "6948": "EAGLES",
    "6956": "SHAM-ROCK-BOTICS ☘",
    "6961": "SIbots",
    "6962": "Antares",
    "6963": "Blood Sweat and Gears",
    "6964": "BearBots (FRC)",
    "6969": "Cyborgs6969",
    "6974": "Zia Robotics",
    "6975": "The Neil McNeil Brotherhood Bots",
    "6978": "QuickStrike Niagara",
    "6981": "Clockwork Concord",
    "6985": "ENKA TECH",
    "6986": "PPT Bots",
    "6988": "ACI35",
    "6989": "KAISER",
    "6995": "NOMAD",
    "6996": "Koalafied",
    "6998": "Unipards",
    "7002": "风云蹦豆",
    "7018": "EOLOTICS - Peñoles",
    "7019": "Rubber Ducks",
    "7028": "Binary Battalion",
    "7030": "The ROBOlympians",
    "7031": "Duanesburg Jr./Sr. High School",
    "7033": "Manna Roosters 7033",
    "7034": "2B Determined",
    "7035": "MEF Thunders",
    "7036": "The Beta Bots",
    "7037": "Jeunes D'Argent - FresnilloPLC",
    "7038": "Totino-Grace Pizza Pi's",
    "7039": "❌⭕",
    "7041": "Doomsday Dogs",
    "7042": "Poly Rabbotics",
    "7045": "MCCrusaders",
    "7048": "Red River Rage",
    "7050": "SARNIC",
    "7054": "SC Sailors",
    "7056": "The F.A.S.T. Team - Fowlerville Area Scientific Technicians",
    "7058": "StrathDroids",
    "7064": "Voltron Robotics",
    "7067": "Team Streak",
    "7068": "Mechanical Masterminds",
    "7072": "O.G.R.E. (Opelika's Greatest Robotics Engineers)",
    "7085": "Wired Warriors",
    "7086": "IOROBOT",
    "7088": "Somerset Robodogs",
    "7091": "KnightOwls",
    "7094": "Alpha Omega",
    "7102": "PrepaTec - Daedalus",
    "7103": "Robo Remedy @ Manitowoc Lutheran High School",
    "7104": "The Bot Brothers",
    "7109": "Clare Chaos Theory",
    "7110": "Heights Bytes",
    "7111": "RAD Robotics",
    "7112": "EverGreen",
    "7113": "Control Rising",
    "7115": "Red Lion Bots",
    "7117": "Robotic Dragons",
    "7119": "Sunset RoboBison",
    "7121": "Keller Fusion Robotics",
    "7125": "Tigerbotics",
    "7127": "LongMetal Robotics",
    "7128": "XLR8",
    "7130": "Future Shock",
    "7135": "System Reset",
    "7137": "Project 212",
    "7138": "Wing It",
    "7145": "RoboHawks",
    "7147": "Rising Falcons",
    "7153": "Aetos Dios (Eagles of Zeus)",
    "7155": "The Robotic Rangers",
    "7157": "μBotics",
    "7160": "Ludington O-Bots",
    "7163": "A.I. Metal Pies",
    "7165": "Perkins Pirates",
    "7166": "Red Thunder Robotics",
    "7174": "Charger Robotics",
    "7177": "Turismo",
    "7178": "Yeti",
    "7183": "W.A.R.",
    "7188": "SaddleBots",
    "7190": "Vancouver Rainstorms",
    "7195": "Twin River Robotics",
    "7196": "Tech Tigers",
    "7197": "Mountie Megabots",
    "7198": "TL Fearsome Gears",
    "7200": "Banting Robotics",
    "7201": "Crobotics",
    "7202": "Cyber Orioles",
    "7203": "Cyber Bearcats",
    "7210": "RoboJackets 7210",
    "7211": "Hollywood",
    "7220": "Steel Falcons",
    "7221": "Viking Robotics",
    "7225": "Mechatronic Mustangs",
    "7226": "Error 404",
    "7230": "Mythic Mechanix",
    "7235": "Red Lake Ogichidaag",
    "7245": "LionBot Robotics",
    "7247": "Magical Smurfs",
    "7248": "Tactical Hams",
    "7250": "OsCODEa 0w|5",
    "7255": "Mechanical Mettle",
    "7256": "Irish Robotics",
    "7257": "Semiconductors",
    "7271": "Hangar 84",
    "7272": "Envirobots",
    "7273": "ZooM 7273",
    "7274": "Brashear Bulls Robotics",
    "7287": "Esquimalt Atom Smashers",
    "7288": "NOMMA D3V1LB0TS",
    "7289": "Blue Eagles",
    "7296": "TeRoBoT",
    "7298": "TOROS",
    "7312": "T3",
    "7314": "Tornadoes",
    "7318": "Nanuet Knightronz",
    "7323": "TriTron",
    "7327": "Metal Jackets",
    "7332": "Vancouver Puddle Jumpers",
    "7333": "Ghost",
    "7334": "Esquimalt Proton Pulverizers",
    "7403": "Lightning Blue Lizards",
    "7407": "Wired Boars",
    "7413": "Plus Ultra",
    "7414": "RetroRobotics",
    "7415": "Jaguar Robotics Sponsored By Feit Electric",
    "7417": "Bionic Bombers",
    "7419": "TECH Support",
    "7421": "PrepaTec - OVERTURE",
    "7425": "Green Valley Robotics",
    "7426": "PAIR OF DICE ROBOTICS",
    "7428": "Gigawatts",
    "7431": "Wingspan",
    "7433": "Iona Fusion",
    "7436": "8-Bit Miners",
    "7439": "Qubit Robotics",
    "7443": "Jag-Wires",
    "7444": "MOON STAR ROBOTICS",
    "7447": "Rōnin Robotics",
    "7451": "AvengerRobotics",
    "7452": "Dupo Tigers",
    "7457": "suPURDUEper Robotics",
    "7459": "Taubatexas Robotics",
    "7461": "Sushi Squad",
    "7466": "Cymurghs",
    "7470": "RAMBOT ROBOTICS",
    "7471": "Robo Montagnards",
    "7472": "R2JESU Robotics",
    "7474": "JXN United",
    "7476": "EOM Robotics 7476",
    "7477": "Giddy GOATS",
    "7479": "Cardinal Robotics",
    "7480": "Machine Mavericks",
    "7483": "Mechanically Challenged",
    "7485": "Mo Co Robo",
    "7491": "Cyber Soldiers",
    "7492": "Turbo Torque",
    "7496": "Robo Indians",
    "7497": "MARAHO",
    "7500": "MARAUDERS",
    "7501": "Golden Gears",
    "7503": "Radicubs",
    "7504": "CyBearBots",
    "7506": "WILDCARDS",
    "7514": "EVE Robotics",
    "7515": "Dark Side Robotics",
    "7516": "Louisville Centrons",
    "7520": "Team MineKee",
    "7521": "ULTIMATE Robotics",
    "7525": "Pioneers",
    "7528": "Nuts and Bolts",
    "7530": "Raiders of the Lost Spark",
    "7531": "Servos Strike Back",
    "7534": "Dragonflies",
    "7536": "ANKA ROBOTICS",
    "7538": "Metal Mountain",
    "7539": "Elev8",
    "7540": "Timberwolf Robotics",
    "7541": "Maple River Robotics 7541",
    "7546": "Prepa Tec - MinerZ",
    "7551": "Extreme Mechanism",
    "7553": "OSTC - SWEET BOTS",
    "7558": "ALT-F4",
    "7563": "SESI SENAI MEGAZORD",
    "7565": "SESI SENAI ROBONÁTICOS",
    "7567": "SESI SENAI OCTOPUS",
    "7570": "New Tech",
    "7576": "FMWill",
    "7577": "VELABOTS - Peñoles",
    "7578": "Quantum Mechanics",
    "7583": "Embers",
    "7587": "Metuchen Momentum",
    "7589": "Lishan Blue Magpie",
    "7594": "Nautilus",
    "7597": "Team HorsePowered",
    "7598": "SCA Constellations",
    "7601": "Shenzhen Unity Stallion",
    "7603": "7603 VESPA ROBOTICS",
    "7605": "ASTRO",
    "7607": "The Blackbirds",
    "7611": "Amal Hawks Robotics",
    "7616": "Cerberus",
    "7617": "RoboBlazers",
    "7619": "illumiBOTi",
    "7627": "Bearcat Robotics",
    "7632": "An Kang Robotics Maker",
    "7634": "New Hawks",
    "7636": "Robomania",
    "7652": "Miami Beach Bots",
    "7653": "Minebots 7653 - Fresnillo PLC -CECyTE 18 Ciénega",
    "7654": "Robot In Shed",
    "7656": "MC Hammers",
    "7657": "Thunderbots",
    "7658": "MagiTech",
    "7659": "HNMCS Robotics",
    "7660": "The Byting Irish",
    "7667": "OtterBots",
    "7668": "GatorBots",
    "7672": "SCORPIONS ROBOTICS",
    "7673": "Formosan Sika",
    "7674": "RaiderBots",
    "7676": "THS RoboDogs",
    "7677": "Bodacious Bulldogs",
    "7687": "Corcebots - Peñoles",
    "7688": "Creekside Robotics",
    "7691": "TSA All Stars",
    "7692": "Beach Bums 7692",
    "7694": "THE IRON LIONS",
    "7695": "Trojan Thunder",
    "7700": "West Tech Paladins",
    "7712": "ACCN UMOJA",
    "7713": "Norse Code",
    "7716": "JLW Formula 01",
    "7717": "Steam Punks",
    "7718": "HVPA Dragons",
    "7724": "Molokai Robotics",
    "7725": "PrepaTec - NUTS & VOLTS",
    "7729": "RAMs",
    "7730": "uNReal",
    "7737": "RaptorBotics",
    "7738": "Helion",
    "7742": "Cosmos Robot Works",
    "7747": "Hancock High Tech",
    "7748": "TECHTOLIA ROBOTICS",
    "7755": "Phantom Circuits",
    "7757": "Atomic Dishwashers",
    "7758": "MEVOLUTION",
    "7759": "Bulldog Bytes",
    "7760": "Griffin Innovations",
    "7762": "AutoPilots",
    "7763": "Carrborobotics",
    "7768": "The AutoMatons",
    "7769": "The CREW",
    "7770": "Infinite Voltage",
    "7772": "The Lucky 7's",
    "7777": "Septobots",
    "7782": "Wolverine Circuit Breakers",
    "7784": "Vassar Voltage",
    "7790": "Baywatch Robotics",
    "7794": "Eagle Engineers",
    "7796": "Breaker Robotics",
    "7797": "RipSaw Robotics",
    "7808": "Kalkaska Dragoneers",
    "7809": "FROST BYTE Robotics",
    "7810": "Eagle Robotics",
    "7813": "New Haven RoboRaptors",
    "7814": "RV Gearheads",
    "7818": "Williamston High School Vespidae",
    "7833": "FALCONS",
    "7840": "EMONER ROBOTICS",
    "7845": "8BIT",
    "7848": "WI-FIghterz",
    "7849": "Da Vinci's Creation",
    "7850": "Bio-Bots",
    "7855": "Chronic Robophobia",
    "7858": "Warriors",
    "7864": "The Bear Bots",
    "7865": "HODAbotics",
    "7885": "Panther Tech",
    "7886": "Cadet Robotics",
    "7890": "SeQuEnCe",
    "7895": "Trobots",
    "7900": "Trial N' Terror",
    "7902": "Markham FireBirds",
    "7903": "Tigers",
    "7906": "Underdogs",
    "7907": "Spartan Robotics",
    "7911": "Scrap Cat Robotics Team Puma",
    "7915": "Ripon Robotics",
    "7917": "Frontier",
    "7918": "ReCuRsIoN",
    "8005": "Mega MeadowBots",
    "8006": "The Phantoms",
    "8011": "Kirin",
    "8012": "APEX Robotics",
    "8013": "Boston Lions",
    "8015": "The Cheetahs",
    "8016": "Eco Robotics",
    "8020": "CyberpunK",
    "8023": "Argonauts",
    "8024": "Robotans",
    "8029": "Steel Bronco Robotics",
    "8032": "SAASquatch Robotics",
    "8033": "Highlander Robotics",
    "8041": "Lake City Ultrabots",
    "8042": "Fenrir",
    "8044": "Denham Venom",
    "8045": "The Wallace Dynasty",
    "8046": "LakerBots",
    "8048": "East Palo Alto Churrobots",
    "8051": "TinkRex",
    "8056": "Laissez Faire",
    "8057": "All Wired Up",
    "8058": "IEL ROBOTICS",
    "8060": "VHS Robotics",
    "8066": "WOLF ARMY ROBOTICS",
    "8067": "Alpha Lab",
    "8069": "Super Hornets",
    "8070": "CyberGriffins",
    "8075": "CyberTigers",
    "8077": "DREAM",
    "8079": "River Robotics Team",
    "8080": "Sequoyah High School Robotics",
    "8081": "UMEI Lightning Robotics",
    "8084": "ALFA ROBOTICS",
    "8085": "MOJO",
    "8088": "Bionic Panthers",
    "8089": "Rockway Robotics",
    "8092": "Greatest of All Times",
    "8096": "Cache Money",
    "8100": "Grayson Robotics",
    "8103": "Knight Robotics",
    "8105": "Apollo Black Cat - Peñoles",
    "8112": "Cyclotrons",
    "8114": "Rocketeers",
    "8115": "Platinum Peacocks",
    "8116": "Hatchet Robotics",
    "8117": "Easton RoboRovers",
    "8119": "TigerBotics",
    "8122": "Mechanical Mustaches",
    "8124": "RHS BearBots",
    "8126": "Control Freaks",
    "8130": "Absegami Robotics",
    "8132": "Sysmik",
    "8137": "Byting Bulldogs",
    "8140": "RoboManiacs",
    "8145": "Tech Warriors",
    "8151": "Wildfire",
    "8152": "Escouade",
    "8153": "Tech4Peace",
    "8158": "Bosphorus Robotics",
    "8159": "Golden Horn",
    "8166": "ROBOCUBE",
    "8169": "Aries",
    "8173": "Brighton College Gear Zero",
    "8175": "Piece of Mind",
    "8177": "Vector",
    "8179": "Robo Falcons",
    "8182": "Loth Robotics",
    "8188": "Grand Force",
    "8193": "Steel Stingers",
    "8205": "Mind Over Metal",
    "8209": "SEZMECH",
    "8214": "Cyber Unicorn",
    "8220": "Space Tigers MBA Robotics",
    "8222": "The Argonauts",
    "8223": "Mariners",
    "8224": "STEM 4 ALL",
    "8229": "Balikbayan Bots",
    "8230": "KoiBots",
    "8234": "Panthinators",
    "8242": "Tekno NFL",
    "8243": "AstroCircuits",
    "8248": "ChainLynx",
    "8255": "Robodogs",
    "8263": "Robin",
    "8264": "ATA ROBOTICS",
    "8266": "Owlbot - Peñoles",
    "8267": "Riptide Robotics 8267",
    "8276": "OPTRON#8276",
    "8280": "K9.0 Robotics",
    "8285": "Bionic Bulldogs",
    "8286": "E.A.G.L.E.S. Robotics",
    "8290": "Jaegers60 - Peñoles",
    "8291": "Golden Yellow Experience - Peñoles",
    "8293": "Prepa Tecmilenio - Startech",
    "8297": "WIHI Robo Wolves",
    "8300": "Titan Robotics",
    "8302": "⚡️ Ｎｉ-ＴＲＯ ⚡️",
    "8303": "GatorBytes",
    "8308": "GFLRobotics",
    "8324": "MECO Robotics",
    "8326": "Hopper Robotics",
    "8331": "Carson Graham Secondary",
    "8334": "Titan Robotics",
    "8338": "Bear Force",
    "8339": "Penticton Robotics",
    "8349": "Newbotics",
    "8352": "CODE Gold",
    "8362": "Circuit Breakers",
    "8364": "Rising Stars Academy - Nebulas",
    "8370": "Glow With The Flow",
    "8372": "HCN Storm",
    "8373": "The Flying Octopi",
    "8374": "Bongo Bots",
    "8385": "Metal Madness",
    "8387": "MECH Mavericks",
    "8393": "The Giant Diencephalic BrainSTEM Robotics Team",
    "8397": "Astrobots",
    "8404": "SKYWALKERS",
    "8408": "Kiss Kats",
    "8410": "Oyster River Overdrive",
    "8421": "CRUSADERS",
    "8422": "Husky Howlers",
    "8423": "Next Express",
    "8424": "Tractor Technicians Next Gen",
    "8426": "Automated Amphibians",
    "8427": "ProtoBots",
    "8429": "Valence Robotics",
    "8430": "The Hatch Batch",
    "8431": "Robo-Knights",
    "8432": "Cyber Knights",
    "8500": "ROBEST Robotics",
    "8503": "Raid One",
    "8507": "Jagwire Robotics",
    "8512": "Manzano Monarchs",
    "8513": "Sisters 1st",
    "8515": "Photon",
    "8516": "Wired Up",
    "8517": "Bounty Hunters",
    "8521": "CyberLions",
    "8528": "AstroChimps",
    "8531": "TermiKnightors",
    "8532": "Classified",
    "8533": "Rancho Reapers",
    "8544": "Reinforcement",
    "8550": "The Ram Bots",
    "8551": "RoNex",
    "8557": "ConquEra",
    "8561": "Cartesian Robotics",
    "8567": "Team Ultraviolet",
    "8569": "Zenith Polaris",
    "8570": "Binary-Bolts",
    "8573": "Rogue-Rangers LEGACY",
    "8575": "The Due Westerners",
    "8576": "Golden Warriors Robotics",
    "8577": "Georgia Cyber Academy Champions",
    "8581": "PizzaByte Robotics",
    "8584": "Infinity Robotics",
    "8585": "Bombax Leopard",
    "8586": "Firebirds Robotics",
    "8588": "Tech Devils",
    "8590": "Cavbotics",
    "8591": "Archangels",
    "8592": "Newton Squared",
    "8595": "This Is How We Play",
    "8602": "STEAMCo",
    "8608": "Alpha Bots",
    "8611": "Cyber Patriots",
    "8612": "Calvin Christian Robotics",
    "8613": "Barker Greybacks",
    "8622": "The CyBirds",
    "8623": "Michigan FC",
    "8624": "Sasquatch Robotics",
    "8626": "Cyber Sailors",
    "8628": "Newark School of Global Studies",
    "8630": "CAP ROBOTICS",
    "8700": "W.I.R.E.D",
    "8701": "GRAFFITI",
    "8704": "Void Warranty",
    "8705": "Sci Tech FRC",
    "8706": "MXS Bulldog Bots",
    "8707": "The Newark Circuit Breakers",
    "8708": "Ov3R1y K0Mp13X",
    "8709": "Pathfinder Tech",
    "8711": "The Midnight Ostrich Runners",
    "8713": "Nordonia Knights",
    "8714": "Robo Griffins'",
    "8717": "Cyber Vipers",
    "8718": "Tigerbotics",
    "8719": "Oakville Dynamics",
    "8721": "Code Name M.I.T.",
    "8724": "Mayhem",
    "8725": "MistyPanther",
    "8726": "CryptoHawks",
    "8727": "Glitch 2.0",
    "8728": "Argonauts",
    "8729": "Sparkling H2O",
    "8732": "Trinity Force",
    "8736": "The Mechanisms",
    "8737": "Rocket Rampage#8737",
    "8738": "Slice",
    "8739": "Redhawk Robotics",
    "8740": "Prepa Tecmilenio - Quantum Force",
    "8741": "Prepa Tecmilenio - Raccoons",
    "8742": "Argyll Attack",
    "8744": "Leviathans",
    "8746": "Knights",
    "8747": "BLACK SEA ROBOTICS",
    "8749": "Farmersville Robotics",
    "8750": "Tarpontron1cs",
    "8751": "Miramonte Robotics Team",
    "8753": "Spectre",
    "8756": "Magic Valley Logic Control",
    "8757": "GeodeBots",
    "8758": "Tar-Robo Turtles",
    "8759": "ARPEX",
    "8761": "AHS TigerByte8761",
    "8764": "Thunder Robotics",
    "8766": "The Black Machine",
    "8767": "Dark Horse Robotics",
    "8768": "CTEC Titans",
    "8769": "G.O.R.T.s",
    "8770": "FireBolts",
    "8771": "PioTech",
    "8777": "SafranTech",
    "8778": "HSUWerx",
    "8780": "TED ROBOTICS",
    "8782": "TroBotics",
    "8783": "Apeiron",
    "8784": "HEVSEL",
    "8787": "Mavericks",
    "8789": "The Lost 10mm Sockets",
    "8790": "Departure",
    "8791": "Mighty Patriots",
    "8793": "Pumpkin Bots",
    "8795": "The Chaotics",
    "8802": "Milwaukee United",
    "8803": "MechaMarines",
    "8805": "NKST36",
    "8806": "Our Lady of Providence Dream League",
    "8808": "SWLA Tech Pirates",
    "8810": "The Alphabots",
    "8815": "Barnstormers",
    "8816": "Coyotronics",
    "8817": "Buccaneer Robotics",
    "8818": "The Foe - Foe Engineering Group",
    "8821": "TVRT-Trojanbots",
    "8822": "Lc3 Thunderbirds",
    "8825": "Team Eagle Eyed",
    "8828": "EAL Robotics",
    "8832": "AutoDawgs",
    "8839": "Shoshone Robotics",
    "8840": "Bay Robotics",
    "8841": "OTTER",
    "8844": "Flamethrowers",
    "8847": "Milwaukee Cyber Cheese",
    "8849": "Storm Robotics",
    "8850": "Ed Bears",
    "8852": "Drew Robotics",
    "8858": "Beast from the East",
    "8859": "Bora Robotics",
    "8863": "ANATOLIAN DEAF ETERNALS ROBOTICS",
    "8865": "Dacula Droids",
    "8866": "Innovation Robotics",
    "8870": "RedCards",
    "8871": "Saber Dynamics",
    "8872": "JJSJF / Gwinnett-NSBE Jr. 'SPECIAL OPS'",
    "8873": "Rockefeller Robotics",
    "8878": "ArrowBot",
    "8880": "Terra Cyborg",
    "8882": "INFINITY BR",
    "8884": "Knight Owls",
    "8885": "Socratic Schematics",
    "8887": "West Central Area",
    "8889": "Project W",
    "8891": "Wild Raccoons",
    "8896": "Wolf Tech Robotics",
    "8898": "Royal Rebel Robotics",
    "8899": "CyberEagles",
    "9001": "Delta Force",
    "9002": "Falcon Dynamics - Foothill High School",
    "9003": "TuTu Turtles",
    "9004": "Milliones Mega Mechatronic Wildcats",
    "9005": "Avian Robotics",
    "9007": "MIT (Muskogee's Innovative Team)",
    "9012": "Team Waffles",
    "9014": "Vulcan Mechanics",
    "9015": "Questionable Engineering",
    "9016": "Syosset SuperSonics",
    "9018": "ElRobotiKO",
    "9019": "Ko'olaupoko",
    "9020": "MCT Galatasaray Robotics",
    "9021": "uSELESScASE",
    "9022": "MC Robotics",
    "9023": "Future Martians",
    "9024": "Callister",
    "9025": "Mechameleons",
    "9026": "Aero",
    "9027": "PATH to Domination",
    "9028": "GFORCE",
    "9029": "Team NF",
    "9030": "Garabagh MecHack",
    "9031": "Wildbots",
    "9032": "RoboWhales",
    "9033": "Ocebots",
    "9034": "The Space Dolphins",
    "9036": "Ramen Robotics",
    "9037": "HHS Black Ops",
    "9038": "SF UNITY",
    "9040": "Sunset Robo Knights",
    "9042": "SOARIN",
    "9043": "Valkyrie",
    "9045": "North Lions SESI/SENAI",
    "9046": "SESI SENAI BBTECH",
    "9047": "TECHMAKER ROBOTICS",
    "9048": "SESI/SENAI P0T1BAT (Potibat)",
    "9049": "SESI SENAI STEAMPUNK MONKEY FRC",
    "9050": "Tucanus",
    "9053": "Prepa Tecmilenio - Iron Hawks",
    "9055": "Knights of Katahdin (Ka TAH din)",
    "9056": "CyberHuskies 9056",
    "9057": "G.I.R.L.",
    "9059": "COLTech Robotics",
    "9060": "Prepa Tecmilenio - Desert Robotics",
    "9061": "Mechanical Mutts",
    "9062": "Critical Circuits",
    "9066": "Cavalo Vendado",
    "9067": "The Goonies",
    "9068": "The Chargers",
    "9071": "SlicerTech",
    "9072": "TigerBots",
    "9073": "Skol Robotics",
    "9074": "Roboant",
    "9075": "Redskin Robotics",
    "9077": "The Crown",
    "9079": "Mechanic storm",
    "9080": "ENIGMA",
    "9082": "No Limit",
    "9084": "Octobots",
    "9085": "SESI SENAI MEGA HARPY",
    "9086": "Wildcat Robotics",
    "9089": "ACS Phoenix",
    "9091": "A.W.E.",
    "9092": "Tower Robotics",
    "9094": "The Earthquakers",
    "9096": "Bad Weather",
    "9097": "MachBusters",
    "9098": "FireHawks",
    "9100": "Robo Roses",
    "9101": "Green Mountain Robotics",
    "9102": "CyborgBulls",
    "9105": "TechnoTalons",
    "9106": "Spires",
    "9110": "SESI SENAI ATOMIIC",
    "9111": "Pyro Mechanics",
    "9114": "Rotationaries",
    "9116": "The Canucks & Bolts",
    "9119": "Iron Legends",
    "9120": "Silverbot - Fresnillo PLC",
    "9121": "Mavericks",
    "9125": "Mechanical Magicians",
    "9126": "Silver Hawks",
    "9128": "ITKAN Robotics",
    "9132": "Basaksehir Teknoloji Takımı",
    "9134": "Tecmilenio - DEVOLT",
    "9136": "Rampage",
    "9137": "Bionic Vipers",
    "9138": "Robo Hawks",
    "9139": "Reserve Robotics",
    "9140": "The Robobots",
    "9146": "Burnouts",
    "9147": "The Joebots",
    "9148": "Atlas Robotics",
    "9149": "RoboCats",
    "9150": "Labyrinth Robotics",
    "9151": "RoboLynx",
    "9152": "Rat Fight",
    "9153": "Bearcat Robotics",
    "9155": "New Tech Kraken",
    "9157": "Iconic Panthers",
    "9160": "YAL THUNDERS",
    "9162": "ALL MIGHT",
    "9163": "HYDRA",
    "9164": "Tech Vikings",
    "9166": "TecRobot",
    "9168": "AGROBOT",
    "9169": "AGROTECH",
    "9175": "BrainMachine-FRC",
    "9176": "Greenspire Steel Sturgeons",
    "9178": "Cyber One",
    "9180": "Ursa Major Robotics",
    "9181": "PlatyPirates",
    "9182": "Tech Tr1be",
    "9183": "RoboComets",
    "9189": "Disastrous Data",
    "9191": "My Chemical Robots",
    "9193": "Short-Circuited Eagles",
    "9194": "Gemin√-1s",
    "9195": "ProdiXy",
    "9198": "The Beach Botz",
    "9199": "SESI SENAI SHARKS",
    "9200": "SESI SENAI STARDUST",
    "9201": "TEAM FORCE",
    "9202": "Silicon Valley Technos",
    "9203": "Quake",
    "9204": "Aluminum Armada",
    "9205": "Technical Marauders1",
    "9206": "GRP Titans",
    "9207": "Redneck Robotics",
    "9208": "Eddies Circuit",
    "9210": "Fighting Falcons",
    "9212": "Infinity",
    "9213": "Tecuani",
    "9214": "Honking Narwhals",
    "9215": "T-Town Tech",
    "9218": "Alpha #9218",
    "9219": "Nine Tails",
    "9220": "Geekos",
    "9226": "GRemlins Gabriel Richard CHS",
    "9231": "Haydarpasa Panthers",
    "9232": "IBBTECH",
    "9233": "Luminous Robotics Team",
    "9234": "Les aigles d'or",
    "9237": "The Dragonators",
    "9238": "Jaybots",
    "9239": "Panthers robotics",
    "9242": "Hamady Hawks",
    "9244": "Belize IT",
    "9245": "Laker Dreadnoughts",
    "9247": "SainTech Robotics",
    "9249": "Copper Country Cats",
    "9251": "EliteBots",
    "9252": "Wingspan",
    "9255": "Botmasterz",
    "9262": "Humberside Huskies",
    "9263": "ALD Lions",
    "9264": "HUNTERS ROBOTICS",
    "9271": "Cyber Porpoises",
    "9277": "Sparkans",
    "9280": "PrepaTec - High Altitude Robotics",
    "9282": "PrepaTec- SINAVOLT",
    "9289": "Vikings",
    "9295": "Harlem Rails",
    "9296": "TITANium",
    "9297": "BOGOBOTS",
    "9298": "Greenies",
    "9300": "Olympians",
    "9301": "PrepaTec - MechRams",
    "9302": "PARATECH FRC",
    "9303": "PORTAL",
    "9304": "legend's",
    "9305": "MonT",
    "9307": "Spicy Chihuahuas",
    "9309": "Metal Mages",
    "9311": "Warhorse Robotics",
    "9312": "NERD Spark",
    "9313": "ROBOhana",
    "9315": "Coding Comets",
    "9316": "CUBATRONICS",
    "9317": "SteamRollers",
    "9400": "BagelBytes",
    "9401": "Midas' Mayhem",
    "9403": "Starstuff",
    "9404": "Forge",
    "9405": "RouxBots",
    "9406": "TechJunior",
    "9408": "Warren Warbots",
    "9410": "PowerHouse",
    "9415": "Westinghouse Robotics",
    "9416": "International Θperatives of World Affairs",
    "9418": "Pastabots",
    "9419": "VULCAN",
    "9421": "Nexus",
    "9424": "E.O. JAG BOTS",
    "9425": "Intra Milwaukee",
    "9426": "CLAWBOTICS",
    "9427": "iDeer",
    "9428": "The Cashew Chickens @ Springfield Robotics",
    "9430": "CTEC Robotics",
    "9431": "Snider Panthers- \"The Gold Standard\"",
    "9433": "Lil' Timmy's Big Team",
    "9437": "Cosmonauts",
    "9438": "Leopard Tech",
    "9439": "Knights of Polaris",
    "9441": "Aero Jr",
    "9442": "Miso Mechanics",
    "9443": "Aluminum Panthers",
    "9444": "Perseverance",
    "9445": "The Brave Bots",
    "9446": "C Cubed",
    "9449": "Yellowjackets",
    "9450": "Velocity Raptors",
    "9451": "Aydos",
    "9452": "LHS Steel Stingers",
    "9453": "Mount Vernon Wildcats",
    "9455": "DinoMights",
    "9456": "Nunez Robotics",
    "9457": "Robot X",
    "9458": "SESI SENAI JACTECH",
    "9459": "SESI SENAI RPRT HAWKS",
    "9460": "SESI SENAI STEEL BULLS",
    "9461": "SESI SENAI MecaChronos",
    "9462": "Toefur's Titans",
    "9465": "Imagine High Axolotls",
    "9467": "EsRobotik",
    "9468": "Team Sirius",
    "9470": "Ctrl-Alt-Defeat",
    "9471": "EF Academy",
    "9472": "K2C",
    "9473": "CYGNUS ROBOTICS",
    "9474": "Red River Resistance (3R)",
    "9475": "Mon Valley Mammoths",
    "9477": "Lambda (λ)",
    "9478": "Robo-Colts HSISL",
    "9479": "CREDENTIS",
    "9480": "The Gear Bears",
    "9483": "Overcharge",
    "9484": "Robot's District",
    "9485": "HYOBOTS",
    "9486": "ALPHASTORM FRC",
    "9487": "KRYPTON FRC",
    "9491": "Crypto Creek",
    "9492": "LadyTalons",
    "9493": "TechWasps923",
    "9494": "Hanabi",
    "9496": "LYNK",
    "9497": "NIMBLE PANTHER",
    "9498": "ZENSHIN Robotics",
    "9499": "KANARITECH - FRESNILLO PLC",
    "9501": "Sidereal Envoy",
    "9502": "Idigna",
    "9503": "EF5 CHAOS",
    "9505": "Vyserions",
    "9506": "Conspiracy Robotics",
    "9509": "Cybears",
    "9512": "Horsepower",
    "9513": "Circuit Crew 9513 - Caterpillar",
    "9519": "RoboMasters",
    "9520": "Rock-it Robotics",
    "9521": "Humble Rumble",
    "9522": "Archimedes",
    "9523": "Archers",
    "9528": "The Wolf Pack",
    "9530": "John Adams Robotics",
    "9532": "Messy Bees",
    "9534": "Wolves Robotics (Basic HS)",
    "9541": "M-stee (squared)",
    "9542": "Technodogs",
    "9543": "Cyclone Robotics",
    "9545": "Caracal Robotics",
    "9548": "Bark River Bronconaughts",
    "9549": "The Mechadogs",
    "9550": "STRIX Robotics",
    "9551": "Tiger",
    "9552": "Rambotics",
    "9554": "Portage Pythons",
    "9555": "TEAM HELIOS",
    "9558": "The Steel Vipers",
    "9559": "EagleTron",
    "9561": "Dawson Dino Nuggets",
    "9562": "Royal Robotics",
    "9566": "Red Storm Rookies",
    "9567": "StormBreakers",
    "9568": "AstroBots",
    "9569": "Milliken Mills Silver Knights",
    "9570": "Valiant",
    "9572": "CC Shambytes",
    "9573": "MarauderTech",
    "9574": "Rock Bots",
    "9575": "Silver Robotics",
    "9577": "Recoil",
    "9578": "Player One",
    "9579": "Tech Devils",
    "9580": "Grizzly Gears",
    "9582": "HHS NASA Snaggbotics",
    "9583": "Lavender Robotics",
    "9584": "Spartan Squared",
    "9585": "RobUL",
    "9586": "Denbot",
    "9589": "Arcade Robotics",
    "9591": "DESTAN",
    "9592": "Blues Robotics",
    "9593": "Rhinos",
    "9594": "Ashley Bears",
    "9595": "ECHO",
    "9597": "Luban Robotics",
    "9600": "X-Flare Robotics",
    "9601": "BageTech",
    "9602": "CANINTECH",
    "9603": "MTECH",
    "9604": "TUIUTECH",
    "9605": "Creek Commanders",
    "9607": "RJR Cyber Demons",
    "9609": "Paraducks",
    "9610": "Blue Ivy",
    "9611": "SESI SENAI SC CyberRain",
    "9613": "Lion's Byte",
    "9614": "ROBOSSAUROS",
    "9615": "Redlands Robotics",
    "9617": "Metal Knight",
    "9618": "Tronarchs",
    "9622": "GRIDLOCK",
    "9624": "Dynamiques 9624",
    "9626": "JagWire Robotics",
    "9627": "Yellow Haze",
    "9631": "Chargers",
    "9635": "Cyber Rabbit",
    "9636": "Space Junk",
    "9638": "Tigers, Bro!",
    "9642": "I.N.T. Robotics",
    "9643": "Lady Voltage",
    "9644": "NEIA Robotics",
    "9646": "The Marinators",
    "9648": "Circuit Breakers",
    "9649": "Mulgrave Robotics",
    "9650": "Mio Thunderbotz",
    "9651": "Ecorse Robo Raiders",
    "9653": "O.V.R.T (Ohio Valley Robotics Team)",
    "9657": "SESI SENAI IPÊV4",
    "9658": "Camber",
    "9659": "Vanier Vikings",
    "9660": "Delgado Community College Dolphinbots",
    "9661": "Huskie STEAM",
    "9662": "Valley Lutheran Chargers",
    "9668": "West Robotics",
    "9669": "Regina Riveters",
    "9670": "The Gadgeteers of St. Johns County",
    "9671": "Robosnacks",
    "9672": "Colon Magi Robotics",
    "9673": "Volt Vespids",
    "9674": "Trojan Power Robotics",
    "9679": "Madison Eagles",
    "9682": "Norway Robotics",
    "9684": "Cardozo Unstoppable",
    "9685": "Crystal Oscillators",
    "9692": "Sigma Powered By Adipro",
    "9694": "TUBITECH",
    "9696": "Hamichanics",
    "9697": "Bionic Bucs",
    "9698": "Zipbombs of BGCNF",
    "9699": "Bayou Crusaders",
    "9703": "Q_Bots",
    "9704": "IgKnighted",
    "9706": "R^3 (Redwood Ranger Robotics)",
    "9708": "Supersonic Circuit Breakers",
    "9709": "Isotope Robotics",
    "9710": "Discovery Polybotz",
    "9715": "Tsinelas'",
    "9717": "Poultronix",
    "9719": "ThunderTech Robotics",
    "9720": "Metal Head Robotics",
    "9721": "Red Hawk Robotics",
    "9722": "Polar Robotics",
    "9726": "Notus Pirates",
    "9727": "Warrior Robotics",
    "9728": "BGCCF Joe R Lee Branch",
    "9729": "Knights",
    "9730": "Metal Maniacs",
    "9731": "All Saints Range Precision",
    "9733": "Gold Diggers",
    "9734": "Jasper County Robotics",
    "9735": "Electric Dragons",
    "9736": "Tawas Braves",
    "9738": "Ionic Bond",
    "9739": "Firefly",
    "9740": "CANBus in memory of Roney Tal",
    "9743": "MV TITANS",
    "9745": "Lazer Sharkz",
    "9747": "ElectroPanthers II",
    "9750": "ED Robotics",
    "9751": "OIA PantherBots",
    "9753": "Team Success",
    "9754": "Vestabots",
    "9755": "Red Devil Robotics",
    "9756": "Robo Panthers",
    "9757": "Mecha Marauders",
    "9758": "Pantron-X",
    "9759": "Forge of Falcon",
    "9760": "Cristo Rey Jesuit Trailblazers",
    "9761": "LSMSA Robotics",
    "9762": "Raccoon Robotics",
    "9767": "Magnificat MagnifiBots",
    "9768": "ROBOKNIGHTS",
    "9770": "Victory Church of Christ Robo Knights",
    "9771": "FPro - Frankfort HS",
    "9772": "Titans Robotics",
    "9773": "Cardinal Robotics",
    "9776": "Gator Gears",
    "9777": "Knights of the Lab Table",
    "9779": "Gladiator Robotics",
    "9782": "Wextech Titans",
    "9785": "Alectrona",
    "9786": "MHS Wolfpack Team 9786",
    "9787": "Fathom Robotics",
    "9788": "R.A.M.",
    "9789": "Platinum Ringleaders",
    "9790": "Crypto Creek II",
    "9970": "Off-Season Demo Team 9970",
    "9971": "Off-Season Demo Team 9971",
    "9972": "Off-Season Demo Team 9972",
    "9973": "Off-Season Demo Team 9973",
    "9974": "Off-Season Demo Team 9974",
    "9975": "Off-Season Demo Team 9975",
    "9976": "Off-Season Demo Team 9976",
    "9977": "Off-Season Demo Team 9977",
    "9978": "Off-Season Demo Team 9978",
    "9979": "Off-Season Demo Team 9979",
    "9980": "Off-Season Demo Team 9980",
    "9981": "Off-Season Demo Team 9981",
    "9982": "Off-Season Demo Team 9982",
    "9983": "Off-Season Demo Team 9983",
    "9984": "Off-Season Demo Team 9984",
    "9985": "Off-Season Demo Team 9985",
    "9986": "Off-Season Demo Team 9986",
    "9987": "Off-Season Demo Team 9987",
    "9988": "Off-Season Demo Team 9988",
    "9989": "Off-Season Demo Team 9989",
    "9990": "Off-Season Demo Team 9990",
    "9991": "Off-Season Demo Team 9991",
    "9992": "Off-Season Demo Team 9992",
    "9993": "Off-Season Demo Team 9993",
    "9994": "Off-Season Demo Team 9994",
    "9995": "Off-Season Demo Team 9995",
    "9996": "Off-Season Demo Team 9996",
    "9997": "Off-Season Demo Team 9997",
    "9998": "Off-Season Demo Team 9998",
    "9999": "Off-Season Demo Team 9999",
    "10000": "Neutron Star",
    "10002": "BotBuilders",
    "10004": "SCAROB (Star City Area Robotics)",
    "10011": "Gridrunners",
    "10014": "REBELLION",
    "10015": "Bubbles",
    "10016": "Absolute Zero",
    "10017": "GPS Robotics",
    "10019": "SESI SENAI MINERSKILLS",
    "10021": "Guerin Catholic Golden Gears",
    "10029": "The Portotypes",
    "10032": "Singularity",
    "10034": "GammaLand",
    "10043": "Conscius Robotics",
    "10045": "Casque Robotics",
    "10050": "Sea Dragons",
    "10059": "Tech On Titans",
    "10063": "S.C.R.A.P.",
    "10064": "HanniBal",
    "10066": "CephaloSquad",
    "10070": "Ghost Bots",
    "10072": "WhirlyBots",
    "10077": "The H.E.I.S.T.",
    "10079": "Arrowdynamics",
    "10101": "Critical Hit",
    "10107": "ThunderBirdz",
    "10108": "Hazard5",
    "10114": "Phantom Talons",
    "10118": "Robonauts EveryBot",
    "10120": "Wulfa the Wolf",
    "10121": "TMSA MechaTiger",
    "10131": "Royal Turtles",
    "10132": "Kuanta Robotics",
    "10133": "Manhattan Monsters",
    "10137": "RoboKats",
    "10138": "VikingBot",
    "10139": "Tsunami",
    "10141": "Turing Robotics",
    "10142": "ANDROMEDA",
    "10143": "Northeast High Viking Robotics",
    "10146": "ROBIS",
    "10147": "ArmBots",
    "10152": "Orbotics",
    "10156": "Marshy Machines",
    "10157": "Roman Robotics",
    "10159": "PHS MAC-ros",
    "10162": "Viking Robotics",
    "10166": "Thunder Vikes",
    "10167": "Jeunes Sans Frontières",
    "10170": "ND Robotics",
    "10172": "Apex Archers",
    "10173": "Minot Syber-Sentinels",
    "10177": "CyberClaws",
    "10180": "MUCİTLER",
    "10181": "ProtoWarriors",
    "10183": "ARC",
    "10185": "Nexify Robotics",
    "10186": "feenx",
    "10190": "UNIMATE - SESI SENAI SC",
    "10202": "NOVATRON",
    "10203": "LANCE",
    "10205": "MAGNETAR",
    "10207": "Doomsday Engineering",
    "10208": "Mayhem Manufacturing",
    "10211": "Cyber Colts",
    "10212": "Circuit Bakers",
    "10213": "BALTA",
    "10214": "Team Doritos",
    "10216": "QUANTUMBEE",
    "10217": "Catahoula Blue Dogs",
    "10219": "Bathtub Chickens",
    "10221": "#1 Sea Bass",
    "10223": "Crimson Jewels",
    "10224": "MAV MECHANICS (NASA)",
    "10225": "Prepa Tecmilenio - PYROX",
    "10228": "Phoenix Robotics",
    "10229": "Event Horizon",
    "10230": "The Splash",
    "10231": "Midnight Circuits",
    "10232": "Killer Kardinals 2",
    "10234": "TEDRA",
    "10236": "REXTED",
    "10243": "Yücel Boru ANATOLIAN WOLVES",
    "10244": "WOLFS ROBOTICS",
    "10245": "WAMS",
    "10246": "ISTAF Team Infinitech",
    "10247": "RedFox",
    "10248": "Iron Byrds",
    "10251": "Chimera",
    "10252": "Mecha Lions",
    "10254": "Regal Seagulls",
    "10256": "Team Scrap Metal",
    "10257": "Vector Space",
    "10260": "RoboJags",
    "10261": "PYRONEERS",
    "10262": "Bionic Buzzers",
    "10263": "Temari Robotics",
    "10264": "STORM",
    "10266": "Hopkins Galvanized Goats",
    "10281": "MechaWolves",
    "10285": "Sacred Heart Outlanders",
    "10287": "Kingdom Prep RC",
    "10291": "MUTUM-X",
    "10295": "FOREST GUARDIANS",
    "10297": "PANTANALBOTS",
    "10298": "LUVER CONTROL",
    "10300": "Obama FRC",
    "10302": "Byte Force",
    "10303": "C-TEC Robotics",
    "10304": "Circuit Breakers",
    "10308": "HiveMind",
    "10309": "Jayhawks Robotics",
    "10312": "ByteForce",
    "10317": "CIA Robotics",
    "10322": "Beyond the Bell",
    "10323": "DeMolay Dynamics",
    "10332": "Carroll Charger Robotics",
    "10333": "Elevation",
    "10336": "BHA Tigers",
    "10337": "Technoka Robotics",
    "10338": "10338 - CircuitMakers",
    "10339": "BrainShift",
    "10340": "ITKAN Girls",
    "10342": "Lost in Translation",
    "10343": "Tera Robotics",
    "10345": "Sirius",
    "10349": "Pontiac Firebirds",
    "10350": "Canberra CIPHER",
    "10353": "HATAY ROBOTICS",
    "10354": "Sting",
    "10356": "Capitech",
    "10361": "Gatorbots",
    "10366": "Builder Bears",
    "10367": "Category 4 Storm Surge",
    "10370": "Squatch Watch",
    "10372": "THE ANTS",
    "10376": "Plutonium Panthers",
    "10377": "Aughtonauts",
    "10380": "The Dragon Hunters",
    "10383": "Robistim",
    "10384": "Sparkanovas",
    "10385": "Cycle-Breakers",
    "10388": "The Iron Wolverine Angels",
    "10390": "GOOGIRL",
    "10392": "Brawley Robocats",
    "10393": "Foxborough High School",
    "10394": "STEAMCo Valley Stars",
    "10396": "Mostra",
    "10400": "DART Doane Academy Robotics Team",
    "10411": "G-Code Generals 10411",
    "10412": "TechBotTronics-1RSNC",
    "10413": "Flux Robotics",
    "10415": "Torchers",
    "10416": "Radical Raccoons",
    "10421": "Baldwin Coded Dragons",
    "10423": "CB Tech",
    "10424": "ANKA ROBOTICS",
    "10425": "WhiteFox",
    "10427": "Kitimat Sentinels",
    "10428": "KAIZEN",
    "10430": "Solar Chill",
    "10434": "CYBEARS",
    "10435": "IRS - Intelligent Robotic Systems",
    "10437": "Amity Robotics Club (ARC)",
    "10439": "Wild Wires",
    "10444": "Ursa Major",
    "10445": "HYPERVOLT",
    "10446": "Circuitron",
    "10449": "Robo Crusaders 1",
    "10452": "RoboLions",
    "10453": "Northmen Robotics",
    "10454": "UPC Raptors",
    "10455": "Bison Dynamics",
    "10456": "Atlantic Trojan Robotics",
    "10466": "Federal Force",
    "10470": "Sancar Robotics",
    "10471": "Gamarobotics",
    "10472": "Bilkent Next",
    "10473": "Team Lance-B-Bot",
    "10474": "Turcia Machina",
    "10476": "Thunderbots",
    "10478": "NEO Robotics",
    "10479": "Powerhouse",
    "10482": "BuzzBots",
    "10490": "Ely Robotics",
    "10493": "Harrison RoboWarriors",
    "10496": "404: Team Not Found",
    "10497": "Bayou Bots",
    "10500": "Lodestar",
    "10502": "HARPIA ROBOTICS",
    "10506": "Crash Test Dummies",
    "10512": "Beyond Grad Greyhounds",
    "10513": "Team Humanity Robo Falcons",
    "10514": "Mixed Métal",
    "10515": "Aluminum Tethys",
    "10516": "Code Red",
    "10517": "Ctrl Alt Defeat",
    "10518": "D.A.D.S",
    "10521": "ERA Robotics",
    "10522": "Sun Prairie School District",
    "10523": "Singularity Robotics",
    "10526": "Orcas",
    "10527": "5-Star",
    "10529": "BATS",
    "10541": "CarbonPulse Robotics",
    "10542": "EAGLE WARRIORS",
    "10547": "United Phoenix",
    "10548": "TechTonic Robotics",
    "10549": "MardinBK Robocretors Team",
    "10550": "35 Soaring Roneagles",
    "10552": "Mazenod Machinists",
    "10553": "Orange Overdrive",
    "10554": "Team VIPER",
    "10558": "Forté",
    "10562": "STEAMex Scorpions",
    "10565": "FARADAY- Prepa Tecmilenio",
    "10576": "ASHINA",
    "10577": "TARMINATORS",
    "10582": "The Technochauns",
    "10583": "Stampede",
    "10584": "Ridge Robotics",
    "10586": "Unearthed",
    "10587": "Windover Wing Nuts",
    "10589": "Binary Crows",
    "10590": "Compass Team",
    "10591": "No Cap",
    "10592": "Stamford Cyber-Hird TSA",
    "10595": "Big Bay de Noc Black Bear Bolts",
    "10598": "TADroid",
    "10599": "RoboBlazers",
    "10600": "Two Steps Ahead",
    "10604": "Demirören Robotics",
    "10605": "Techno Talons",
    "10606": "The Foley Flare",
    "10607": "DAAS Jaguars",
    "10611": "Gemini Robotics",
    "10612": "Cougear",
    "10613": "AEA Alterbots",
    "10614": "AEA Robolution",
    "10615": "Spartan110",
    "10616": "Iron Eagle Robotics",
    "10619": "Rana's Rivets",
    "10621": "Crusaders",
    "10622": "Gwinn ModelBots",
    "10623": "AY YILDIZ ROBOTİM",
    "10625": "BuzzFusion",
    "10626": "Smokin Batteries",
    "10629": "Bath Bee-Botics",
    "10633": "BeeBotics",
    "10635": "Merengue",
    "10642": "Comstock Colts",
    "10644": "West Coast Warriors",
    "10650": "Robonoles Robotics Team",
    "10651": "Gear Cats",
    "10652": "Mechanical MacGyvers",
    "10654": "Tiger Bytes",
    "10655": "PHS Spartan Synergy",
    "10656": "RataToaster",
    "10658": "SEED Robotics",
    "10659": "Jaguar Byte",
    "10661": "Blue Phoenix",
    "10664": "Big Reds",
    "10665": "Unibots",
    "10666": "Green MachineZ",
    "10667": "Genesee Wolves",
    "10672": "Trojans",
    "10675": "Energize",
    "10679": "The Wirewolves",
    "10682": "Robo-Yotes",
    "10686": "Northfield Nightbots",
    "10689": "KAGITHANE TEKNOLOJI TAKIMI",
    "10690": "ERROR 448",
    "10692": "GOLDEN APPLE",
    "10695": "Galileo",
    "10704": "Storm Surge",
    "10711": "FORMSHOP",
    "10714": "RAMpage.AI",
    "10903": "The Ionizers",
    "10904": "The Circuit Sorcerers",
    "10905": "Team LOVE",
    "10906": "Lunea Lumen",
    "10907": "ROBOLYNX",
    "10909": "CuttingEdge1881",
    "10910": "The Outlaws",
    "10911": "T3 ROBOTICS",
    "10912": "Dragons",
    "10913": "Robot Wranglers",
    "10914": "Team Variant",
    "10917": "M.A.P.L.E.",
    "10918": "Bluesteel Dragons",
    "10920": "F²",
    "10922": "Amped",
    "10924": "Titanium Titans",
    "10929": "Tarpon Robotics",
    "10931": "Silver Talon",
    "10932": "Tulpar",
    "10933": "Tie Dye Samurai",
    "10935": "Krono",
    "10936": "Hooverstan",
    "10937": "River Valley RADs+",
    "10940": "AVIATRONIX",
    "10943": "WJH RoboStangs",
    "10945": "ELP's Mad CAD-ers",
    "10947": "Antioch Rising",
    "10949": "Metalheads",
    "10950": "The Northern Knights",
    "10951": "Saigon South Dragons",
    "10953": "LYRON",
    "10958": "SWing",
    "10959": "ORHANIYE R SPORTS",
    "10960": "Eagle Engineers",
    "10961": "Hyper Wave",
    "10973": "Sea Raiders",
    "10978": "Complete Robotics",
    "10979": "Tiger Robotics",
    "10980": "The Deadbolts",
    "10981": "Jo-Bees",
    "10986": "Negev Dominion",
    "10987": "The Legion",
    "10988": "Desert Storm",
    "10989": "Blast Robotics",
    "10991": "PioBytes",
    "10993": "EP Robotics",
    "10994": "SSES Phenoms",
    "10995": "ACS Eagle Robotics",
    "10996": "SRC Robotics",
    "10997": "St. George",
    "10998": "VOLTARIA ROBOTICS",
    "10999": "Cezeri Robotics",
    "11000": "NEOCHIRON",
    "11002": "Montcalm Robotics",
    "11003": "maui cap",
    "11010": "Bobcats",
    "11019": "X.PLORE",
    "11022": "RISING STAR ROBOTIC TEAM",
    "11024": "Hypernova Robotics",
    "11025": "Garage Gang",
    "11030": "Bonac Bots",
    "11031": "Golden STEAMex",
    "11036": "ALEV ROBOTICS",
    "11037": "Clarksville Cyber Storm",
    "11040": "Aria STEAMex",
    "11064": "ASTERIA",
    "11065": "STEAMex Overheat",
    "11068": "Thronex",
    "11070": "RoboRex",
    "11090": "Kronos",
    "11091": "JMA Patriots",
    "11095": "The Flying Cats",
    "11096": "Δt",
    "11097": "TROPICAL FORCE",
    "11098": "MATRIX",
    "11099": "Capella Robotics",
    "11100": "Team Epsilon",
    "11102": "Real Steel",
    "11103": "Redshift Robotics",
    "11104": "Phoenix Phanatics",
    "11105": "GC 4 Tomorrow",
    "11107": "FARMBots",
    "11118": "The Baybies",
    "11120": "Frostbite Robotics",
    "11130": "Totally Clucked Robotics",
    "11131": "Brainstorm",
    "11132": "Rambotics, Inc.",
    "11138": "TEKNOROB",
    "11139": "Blackhawk Robotics #11139",
    "11141": "AlphaZ",
    "11146": "CitrusDuckz",
    "11149": "Blue Lightning",
    "11154": "XR-Boot",
    "11157": "Winnacunnet",
    "11158": "Superior Robotics",
    "11161": "SkyForge Griffins",
    "11164": "Byte Robotics",
    "11166": "LionTECH",
    "11167": "Team Titan",
    "11169": "ERA Delta",
    "11171": "Hi Gear",
    "11173": "Star Force",
    "11174": "Danes Robotics",
    "11175": "Tidal Shock",
    "11176": "RoboRabbits",
    "11177": "HCSH Leopard Cat",
    "11178": "Revelation",
    "11179": "The Power of Friendship",
    "11188": "SAR Dynamics",
    "11195": "VOLT",
    "11200": "Panther Prime",
    "11201": "MangTech",
    "11210": "MACHS Raiders 1",
    "11211": "Apex Dimension",
    "11212": "BioHazard",
    "11214": "[CY]BEARCATS robotics",
    "11215": "Les Panthères",
    "11216": "FLAZIA ROBOTICS",
    "11219": "North Union Robotics",
    "11220": "The Royal Guard",
    "11221": "Edna Karr Robotics",
    "11222": "STAXX, Sponsored by NASA",
    "11223": "The Whistlepunks",
    "11224": "Azreal",
    "11225": "Wolfimus",
    "11227": "Goose Goose Duck",
    "11229": "Open Circuit",
    "11230": "White Hat Hackers",
    "11235": "Golden Ratio",
    "11240": "Rotatech",
    "11241": "PRAIRIE",
    "11242": "Golden Eagle Mechanics",
    "11243": "Cyber Knights",
    "11244": "Avrasya Robotics",
    "11246": "Tiger Botz",
    "11247": "Alisal High School Robotics Team",
    "11251": "Alpha Phi Alpha OBL",
    "11252": "1st ACT",
    "11254": "FIRST Ignition",
    "11255": "KAR Robotics",
    "11256": "Sattellites",
    "11257": "Hi-Sac",
    "11258": "Honey Badgers",
    "11260": "Ursuline Robotics",
    "11261": "Mount Toro High School Robotics Team",
    "11265": "Rockbotics -FRC",
    "11266": "Finaltech",
    "11267": "MASAL",
    "11269": "Batteries Not Included",
    "11270": "Nova",
    "11274": "Warrior Robotics",
    "11275": "Defenders 1",
    "11276": "Kenner BruhBots",
    "11277": "ARC",
    "11281": "Phoenix Robotics",
    "11282": "Tri-Force",
    "11283": "Melrose Gear Grinders",
    "11285": "Purple Autobots",
    "11287": "Canton Rising Stars",
    "11288": "Trident Force",
    "11292": "Icebots 2",
    "11296": "Merced Fusion Robotics",
    "11297": "Iron Corsairs",
    "11300": "ZENITH",
    "11302": "Hot Rods",
    "11303": "BEZOAR ROBOTICS",
    "11304": "Meowbotics - Fresnillo PLC",
    "11308": "Daniel Morgan Patriots",
    "11309": "Highlands High School Robotics",
    "11311": "The Stags",
    "11312": "Royal Flush",
    "11313": "tROBOmakEr",
    "11318": "Freedom Robotics",
    "11319": "Polaris",
    "11321": "Purple Knight Robotics",
    "11328": "SIA",
    "11329": "G-Force",
    "11331": "TarryBots",
    "11332": "Sunrise",
    "11335": "MARETECH",
    "11336": "Anomaly",
    "11337": "Mystery Machines",
    "11339": "Cibatronic",
    "11342": "Scotlandville Magnet Robotics",
    "11343": "JPS High School Robotics",
    "11345": "Long Beach RoboBearcats",
    "11347": "Bronco Brigade",
    "11350": "Guilford Gearheads",
    "11351": "LYNTECH - Fresnillo PLC",
    "11352": "Flying Tiger",
    "11353": "Visionary Sparks",
    "11354": "Access2Innovate Foundation",
    "11361": "ORA Raven Bots",
    "11362": "RoBumpers",
    "11363": "TBD",
    "11365": "Ignite Robotics",
    "11366": "Red Devil's Redemption",
    "11368": "SCTC ZettaBots",
    "11371": "Odyssey",
    "11377": "Knightmare Robotics",
    "11379": "TURTLESS",
    "11380": "CECYBOTS - Peñoles",
    "11382": "INTERHERMES",
    "11386": "Climax-Scotts PantherBots",
    "11387": "G.L.E.A.M. - Grass Lake Engineering and Mechanics",
    "11388": "STEAM & IRON ROBOTICS",
    "11389": "ŞAL RoboRage",
    "11390": "Habsor",
    "11391": "Hypatia",
    "11392": "Gulf Tech",
    "11398": "Shark Bytes",
    "11399": "Timber Tech",
    "11401": "Glorious Robotics",
    "11404": "The Power of Skopje",
    "11405": "RoboHawks",
    "11407": "Sturgis Trojans",
    "11409": "Alchemist's Return",
    "11410": "WGSS Delta Lab",
    "11415": "Storm Surge",
    "11416": "Raptoris Robotics",
    "11417": "Nano Knights",
    "11423": "Team IIA",
    "11424": "Boa Constructors",
    "11425": "Byteforge",
    "11428": "Corpus Christi",
    "11429": "NEW AI",
    "11431": "LPS Tigers",
    "11435": "St. Peter's High School RaiderBots",
    "11436": "CougarBots",
    "11437": "The ARC - Addison Robotics Club",
    "11440": "Casman Wildcats",
    "11442": "FEKAL ROBOTICS",
    "11443": "Ferrum Nexus",
    "11444": "Cougar Claw Robotics",
    "11453": "FutureX Robotics",
    "11455": "Britton Deerfield Schools Robotics Club",
    "11456": "UMWACC Ruff Creek RoboMiners",
    "11457": "INVEST",
    "11458": "MechCats",
    "11459": "St. Augustine Robotics",
    "11460": "Olivet Eaglebots",
    "11461": "SW Phoenix",
    "11462": "Morenci Robotics 1",
    "11463": "Sacred Heart Cathedral Prep Robotics",
    "11464": "Bristol Eastern",
    "11465": "Hawikku Robotics",
    "11466": "Mega-CAPs",
    "11467": "HSHP 1HeartBeat",
    "11468": "Lisbon Panther Robotics",
    "11469": "Ignis Mechanica",
    "11471": "Neon",
    "11473": "Wixom Christian Warrior Robotics",
    "11474": "SEED Bulldogs Byte!",
    "11475": "DawgBots",
    "11478": "KasimTec",
    "11479": "Formosa Robotics",
    "11480": "EXODUS",
    "11483": "Red Line Robotics",
    "11485": "Bestarian NextGEN",
    "11486": "Chesterton Academy OLOG Robotics",
    "11489": "Ballou Robotic Knights",
    "11490": "The Southwest Sparks",
    "11491": "Bloomingdale Cardinals",
    "11493": "Pixel Knights",
    "11496": "Jurassic JayHawks",
    "11501": "Jackets",
    "11503": "TALONS",
    "11505": "Kensington Woods Robotics",
    "11511": "GearHeads",
    "11522": "Union HS",
    "11527": "FCCA Chrome Crew",
};

const SCRIPT_URL = "https://script.google.com/macros/s/AKfycbyXn4Mg7BqH8f-KfDuZe77Wrf-JLUUrLtArWPhYjtdO-Q-wvtcJi3qP7_rW88yoJQCu7A/exec"; // Your URL

async function syncToSheets() {
  try {
    if (!navigator.onLine) {
      alert('❌ No internet connection');
      return
    }
    
    const matchCount = Number(localStorage.getItem("MatchCount")) || 0;
    const pitCount = Number(localStorage.getItem("PitCount")) || 0;
    
    if (matchCount === 0 && pitCount === 0) {
      alert('No data to sync');
      return;
    }
    
    // Collect matches - PROPER CSV PARSING
    const matches = [];
    for (let i = 0; i < matchCount; i++) {
      const csv = localStorage.getItem("Match" + i);
      if (csv) {
        matches.push(parseCSVRow(csv));
      }
    }
    
    // Collect pits - PROPER CSV PARSING
    const pits = [];
    for (let i = 0; i < pitCount; i++) {
      const csv = localStorage.getItem("Pit" + i);
      if (csv) {
        pits.push(parseCSVRow(csv));
      }
    }
    
    // Send to Google Sheets
    const response = await fetch(SCRIPT_URL, {
      method: 'POST',
      body: JSON.stringify({ matches, pits })
    });
    
    const result = await response.json();
    
    if (result.success) {
      alert(`✓ Synced ${result.matches} matches, ${result.pits} pits`);
    } else {
      alert('❌ Sync failed: ' + result.error);
    }
    
  } catch (error) {
    alert('❌ Sync failed: ' + error.message);
  }
}

// Proper CSV parser that handles quoted fields with commas
function parseCSVRow(csvString) {
  const result = [];
  let current = '';
  let inQuotes = false;
  
  for (let i = 0; i < csvString.length; i++) {
    const char = csvString[i];
    const nextChar = csvString[i + 1];
    
    if (char === '"') {
      if (inQuotes && nextChar === '"') {
        // Escaped quote ("")
        current += '"';
        i++; // Skip next quote
      } else {
        // Toggle quote state
        inQuotes = !inQuotes;
      }
    } else if (char === ',' && !inQuotes) {
      // End of field
      result.push(current);
      current = '';
    } else {
      current += char;
    }
  }
  
  // Add last field
  result.push(current);
  
  return result;
}

function getTeamName(teamNumber) {
    const name = teamData[String(teamNumber)];
    return name || 'Unknown Team';
}

function compareTeamNumber (teamNumber) {
    if (!teamNumber) {
        return false;
    }
    const trimmed = teamNumber.trim();

    if (!/^[0-9]+$/.test(trimmed)) {
        return false;
    }

    const key = String(parseInt(trimmed, 10));
    return Object.prototype.hasOwnProperty.call(teamData, key);
}
    
function loadUsername() {
    const saved = localStorage.getItem(USERNAME_KEY);
    if (saved) {
        document.getElementById('userName').value = saved;
    }
}

function saveUsername() {
    const val = document.getElementById('userName').value.trim();
    if (val) {
        localStorage.setItem(USERNAME_KEY, val);
    } else {
        localStorage.removeItem(USERNAME_KEY);
    }
}

document.addEventListener('DOMContentLoaded', function() {
    loadUsername();
    document.getElementById('userName').addEventListener('input', saveUsername);
});

function updateAutoEstimate() {
    const total = parseInt(document.getElementById('allianceAutoPoints').value) || 0;
    const percent = parseInt(document.getElementById('autoContribution').value) || 0;
    const estimate = Math.round(total * percent / 100);
    document.getElementById('autoFuelScored').value = estimate;
    updateAutoMissed();
}

function updateTeleopEstimate() {
    const total = parseInt(document.getElementById('allianceTotalPoints').value) || 0;
    const percent = parseInt(document.getElementById('teleopContribution').value) || 0;
    const estimate = Math.round(total * percent / 100);
    document.getElementById('teleopFuelScored').value = estimate;
    updateTeleopMissed();
}

function updateAutoScoredFromManual() {
    const scored = parseInt(document.getElementById('autoFuelScored').value) || 0;
    const allianceTotal = parseInt(document.getElementById('allianceAutoPoints').value) || 0;
    
    if (allianceTotal > 0) {
        let percent = Math.round((scored / allianceTotal) * 100);
        // Cap at 100%
        percent = Math.min(100, percent);
        // Round to nearest 5 to match slider steps
        percent = Math.round(percent / 5) * 5;
        
        // Update slider position and display
        document.getElementById('autoContribution').value = percent;
        document.getElementById('auto-contribution-val').textContent = percent;
    }
    
    updateAutoMissed();
}

function updateTeleopScoredFromManual() {
    const scored = parseInt(document.getElementById('teleopFuelScored').value) || 0;
    const allianceTotal = parseInt(document.getElementById('allianceTotalPoints').value) || 0;
    
    if (allianceTotal > 0) {
        let percent = Math.round((scored / allianceTotal) * 100);
        percent = Math.min(100, percent);
        percent = Math.round(percent / 5) * 5;
        
        document.getElementById('teleopContribution').value = percent;
        document.getElementById('teleop-contribution-val').textContent = percent;
    }
    
    updateTeleopMissed();
}

function updateAutoMissedFromManual() {
    const scored = parseInt(document.getElementById('autoFuelScored').value) || 0;
    const missed = parseInt(document.getElementById('autoFuelMissed').value) || 0;
    
    const totalShots = scored + missed;
    
    // Reset placeholder
    document.getElementById('autoFuelMissed').placeholder = 'Shots missed';
    
    if (totalShots === 0) {
        document.getElementById('autoAccuracy').value = 50;
        document.getElementById('auto-accuracy-val').textContent = '50';
        return;
    }
    
    let accuracy = Math.round((scored / totalShots) * 100);
    accuracy = Math.round(accuracy / 5) * 5; // Snap to nearest 5%
    
    // Don't allow 0% if there are scored shots
    if (accuracy === 0 && scored > 0) {
        accuracy = 5; // Minimum 5% if they scored anything
    }
    
    document.getElementById('autoAccuracy').value = accuracy;
    document.getElementById('auto-accuracy-val').textContent = accuracy;
}

function updateTeleopMissedFromManual() {
    const scored = parseInt(document.getElementById('teleopFuelScored').value) || 0;
    const missed = parseInt(document.getElementById('teleopFuelMissed').value) || 0;
    
    const totalShots = scored + missed;
    
    // Reset placeholder
    document.getElementById('teleopFuelMissed').placeholder = 'Shots missed';
    
    if (totalShots === 0) {
        document.getElementById('teleopAccuracy').value = 50;
        document.getElementById('teleop-accuracy-val').textContent = '50';
        return;
    }
    
    let accuracy = Math.round((scored / totalShots) * 100);
    accuracy = Math.round(accuracy / 5) * 5; // Snap to nearest 5%
    
    // Don't allow 0% if there are scored shots
    if (accuracy === 0 && scored > 0) {
        accuracy = 5; // Minimum 5% if they scored anything
    }
    
    document.getElementById('teleopAccuracy').value = accuracy;
    document.getElementById('teleop-accuracy-val').textContent = accuracy;
}
function updateAutoMissed() {
    const scored = parseInt(document.getElementById('autoFuelScored').value) || 0;
    const accuracy = parseInt(document.getElementById('autoAccuracy').value);
    
    // If accuracy is 0 and there are scored shots, don't auto-calculate
    if (accuracy === 0 && scored > 0) {
        // Clear the missed field to force manual entry
        document.getElementById('autoFuelMissed').value = '';
        document.getElementById('autoFuelMissed').placeholder = 'Enter missed shots';
        return;
    }
    
    if (scored === 0) {
        document.getElementById('autoFuelMissed').value = 0;
        return;
    }
    
    const missed = Math.round(scored * (100 - accuracy) / accuracy);
    document.getElementById('autoFuelMissed').value = missed;
    document.getElementById('autoFuelMissed').placeholder = 'Shots missed';
}

function updateTeleopMissed() {
    const scored = parseInt(document.getElementById('teleopFuelScored').value) || 0;
    const accuracy = parseInt(document.getElementById('teleopAccuracy').value);
    
    // If accuracy is 0 and there are scored shots, don't auto-calculate
    if (accuracy === 0 && scored > 0) {
        // Clear the missed field to force manual entry
        document.getElementById('teleopFuelMissed').value = '';
        document.getElementById('teleopFuelMissed').placeholder = 'Enter missed shots';
        return;
    }
    
    if (scored === 0) {
        document.getElementById('teleopFuelMissed').value = 0;
        return;
    }
    
    const missed = Math.round(scored * (100 - accuracy) / accuracy);
    document.getElementById('teleopFuelMissed').value = missed;
    document.getElementById('teleopFuelMissed').placeholder = 'Shots missed';
}


// Switch between Scouting, Pit and History tabs
function switchMainTab(tabName) {
    // Only select the main tabs (first .tabs container that's a direct child of body)
    const mainTabsContainer = document.querySelector('body > .tabs');
    const mainTabs = mainTabsContainer.querySelectorAll('.tab');
    
    // Select only main content sections (scouting, pit, history)
    const mainContents = document.querySelectorAll('body > .tab-content');
    
    mainTabs.forEach(tab => tab.classList.remove('active'));
    mainContents.forEach(content => content.classList.remove('active'));
    
    event.target.classList.add('active');
    document.getElementById(tabName).classList.add('active');
}

// Switch between Pit, Auto, Teleop, and Engame tabs
function switchTab(tabName) {
    const tabs = document.querySelectorAll('#scouting .tabs .tab');
    const contents = document.querySelectorAll('#scouting .tab-content');
    
    tabs.forEach(tab => tab.classList.remove('active'));
    contents.forEach(content => content.classList.remove('active'));
    
    event.target.classList.add('active');
    document.getElementById(tabName).classList.add('active');
}

// Increment fuel buttons 
function increment(buttonID){ 
    const value = document.getElementById(buttonID);

    value.textContent = parseInt(value.textContent) + 1;        
}

// Decrement fuel buttons 
function decrement(buttonID){ 
    const value = document.getElementById(buttonID);

    if(value.textContent > 0){
        value.textContent = parseInt(value.textContent) -1;
    }
}

// Increment fuel buttons by 5 at once
function increment5(buttonID) {
    const value = document.getElementById(buttonID);

    value.textContent = parseInt(value.textContent) + 5;
}

// Decrement fuel buttons by 5 at once
function decrement5(buttonID) {
    const value = document.getElementById(buttonID);

    if(value.textContent - 5 >= 0) {
        value.textContent = parseInt(value.textContent) -5;
    }
    else{
        value.textContent = parseInt('0');
    }
}

function resetForm(){
    
    // Reset counter buttons (textContent elements)
    const counterIds = [
        'teleopFouls',
        'humanPlayerFuelMissed',
        'humanPlayerFuelScored'
    ];
    counterIds.forEach(id => {
        const el = document.getElementById(id);
        if (el) el.textContent = '0';
    });

    // Reset sliders and their display values
    document.getElementById('autoContribution').value = 33;
    document.getElementById('auto-contribution-val').textContent = '33';
    document.getElementById('autoAccuracy').value = 50;
    document.getElementById('auto-accuracy-val').textContent = '50';
    
    document.getElementById('teleopContribution').value = 33;
    document.getElementById('teleop-contribution-val').textContent = '33';
    document.getElementById('teleopAccuracy').value = 50;
    document.getElementById('teleop-accuracy-val').textContent = '50';
    
    document.getElementById('teleopCollaboration').value = 1;
    document.getElementById('collab-val').textContent = '1';
    document.getElementById('drivingScore').value = 1;
    document.getElementById('drive-val').textContent = '1';
    document.getElementById('pitBuildQuality').value = 5;
    document.getElementById('build-quality-val').textContent = '5';

    // Reset fuel input fields (these are now <input type="number">)
    document.getElementById('autoFuelScored').value = '0';
    document.getElementById('autoFuelMissed').value = '0';
    document.getElementById('teleopFuelScored').value = '0';
    document.getElementById('teleopFuelMissed').value = '0';
    
    // Reset alliance points fields
    document.getElementById('allianceAutoPoints').value = '';
    document.getElementById('allianceTotalPoints').value = '';
    
    // Reset fuel input placeholders
    document.getElementById('autoFuelMissed').placeholder = 'Shots missed';
    document.getElementById('teleopFuelMissed').placeholder = 'Shots missed';

    // Reset all checkboxes
    const allCheckboxes = document.querySelectorAll('input[type="checkbox"]');
    allCheckboxes.forEach(checkbox => {
        checkbox.checked = false;
    });

    // Reset all radio buttons
    const allRadios = document.querySelectorAll('input[type="radio"]');
    allRadios.forEach(option => {
        option.checked = false;
    });

    // Reset all text/number fields and textareas EXCEPT userName
    const allText = document.querySelectorAll('input[type="text"], input[type="number"], textarea');
    allText.forEach(text => {
        if (text.id !== 'userName') {
            text.value = '';
        }
    });

    // Reset all dropdowns EXCEPT userName select
    const allDropdowns = document.getElementsByTagName('select');
    for (let i = 0; i < allDropdowns.length; i++){
        if (allDropdowns[i].id !== 'userName') {
            allDropdowns[i].selectedIndex = 0;
        }
    }
}

// Show the unified validation modal.
function showValidationModal(required, warnings, onValid) {
    const modal = document.getElementById('validationModal');
    const requiredSection = document.getElementById('validationRequiredSection');
    const warningSection = document.getElementById('validationWarningSection');
    const requiredList = document.getElementById('validationRequiredList');
    const warningList = document.getElementById('validationWarningList');
    const saveAnywayBtn = document.getElementById('validationSaveAnyway');

    requiredList.innerHTML = "";
    warningList.innerHTML = "";

    if (required.length > 0) {
        requiredSection.style.display = '';
        required.forEach(function(msg) {
            const li = document.createElement('li');
            li.textContent = msg;
            li.className = 'error-item';
            requiredList.appendChild(li);
        });
    } else {
        requiredSection.style.display = 'none';
    }

    if (warnings.length > 0) {
        warningSection.style.display = '';
        warnings.forEach(function(msg) {
            const li = document.createElement('li');
            li.textContent = msg;
            li.className = 'warning-item';
            warningList.appendChild(li);
        });
    } else {
        warningSection.style.display = 'none';
    }

    // Only show Save Anyway if there are no blocking errors
    if (required.length === 0 && warnings.length > 0) {
        saveAnywayBtn.style.display = '';
        _pendingSaveCallback = onValid;
    } else {
        saveAnywayBtn.style.display = 'none';
        _pendingSaveCallback = null;
    }

    modal.classList.add('active');
}

function closeValidationModal() {
    document.getElementById('validationModal').classList.remove('active');
    _pendingSaveCallback = null;
}

function saveAnywayFromModal() {
    document.getElementById('validationModal').classList.remove('active');
    if (_pendingSaveCallback) {
        _pendingSaveCallback();
    }
    _pendingSaveCallback = null;
}

function checkNonAscii(fields) {
    var errors = [];
    for (var i = 0; i < fields.length; i++) {
        var el = document.getElementById(fields[i].id);
        if (!el) continue;
        var val = el.value;
        var match = val.match(/[^\x00-\x7F]/);
        if (match) {
            errors.push(fields[i].label + " contains a special character: \" " + match[0] + " \" — please retype it using a normal keyboard character.");
        }
    }
    return errors;
}
function validateMatch(onValid) {
    const required = [];
    const warnings = [];

    var asciiErrors = checkNonAscii([
        { id: "endgameNotes",     label: "Endgame Notes" },
        { id: "humanPlayerNotes", label: "Human Player Notes" }
    ]);
    for (var i = 0; i < asciiErrors.length; i++) {
        required.push(asciiErrors[i]);
    }

    const scoutName = document.getElementById('userName').value.trim();
    if (!scoutName) {
        required.push("Scout name is required.");
    } else if (!/^[a-zA-Z\s]+$/.test(scoutName)) {
        required.push("Scout name should only contain letters — no numbers or special characters.");
    }

    const matchStageChecked = document.querySelector('input[name="matchStage"]:checked');
    if (!matchStageChecked) {
        required.push("Match stage is required — select Playoffs or Qualification.");
    }

    const driverStation = document.getElementById('driverStation').value;
    if (!driverStation) {
        required.push("Driver Station is required.");
    }

    const matchNumVal = document.getElementById('matchNumber').value.trim();
    if (!matchNumVal) {
        required.push("Match number is required.");
    } else if (parseInt(matchNumVal) > 100) {
        required.push("Match number cannot exceed 100.");
    }

    const teamNumVal = document.getElementById('teamNumber').value.trim();
    if (!teamNumVal) {
        required.push("Team number is required.");
    } else if (parseInt(teamNumVal) > 12000) {
        required.push("Team number cannot exceed 12000.");
    } else if (!compareTeamNumber(teamNumVal)) {
        required.push("Team number must be one of the known teams (enter a valid numeric ID).");
    }

    // ═══ AUTO FUEL VALIDATION ═══
    const autoScored = parseInt(document.getElementById('autoFuelScored').value) || 0;
    const autoMissed = document.getElementById('autoFuelMissed').value.trim();
    const autoAccuracy = parseInt(document.getElementById('autoAccuracy').value);

    if (autoScored > 0) {
        if (autoMissed === '') {
            required.push("Auto: Fuel Missed is required when Fuel Scored > 0 (Auto tab).");
        } else if (autoAccuracy === 0) {
            required.push("Auto: Shot Accuracy cannot be 0% when Fuel Scored > 0 (Auto tab).");
        }
    }

    if (autoMissed !== '') {
        const autoMissedNum = parseInt(autoMissed);
        if (autoScored === 0 && autoMissedNum > 0) {
            required.push("Auto: Cannot have missed shots if Fuel Scored is 0 (Auto tab).");
        }
    }

    // ═══ TELEOP FUEL VALIDATION ═══
    const teleopScored = parseInt(document.getElementById('teleopFuelScored').value) || 0;
    const teleopMissed = document.getElementById('teleopFuelMissed').value.trim();
    const teleopAccuracy = parseInt(document.getElementById('teleopAccuracy').value);

    if (teleopScored > 0) {
        if (teleopMissed === '') {
            required.push("Teleop: Fuel Missed is required when Fuel Scored > 0 (Teleop tab).");
        } else if (teleopAccuracy === 0) {
            required.push("Teleop: Shot Accuracy cannot be 0% when Fuel Scored > 0 (Teleop tab).");
        }
    }

    if (teleopMissed !== '') {
        const teleopMissedNum = parseInt(teleopMissed);
        if (teleopScored === 0 && teleopMissedNum > 0) {
            required.push("Teleop: Cannot have missed shots if Fuel Scored is 0 (Teleop tab).");
        }
    }

    // ═══ ALLIANCE POINTS VALIDATION ═══
    const allianceAutoPoints = parseInt(document.getElementById('allianceAutoPoints').value) || 0;
    const autoContribution = parseInt(document.getElementById('autoContribution').value);
    
    if (autoScored > 0 && allianceAutoPoints === 0) {
        warnings.push("Auto: Alliance Auto Points is 0 but robot scored fuel (Auto tab).");
    }
    
    if (allianceAutoPoints > 0 && autoScored > allianceAutoPoints) {
        required.push("Auto: Robot's Fuel Scored (" + autoScored + ") cannot exceed Alliance Auto Points (" + allianceAutoPoints + ") (Auto tab).");
    }

    const allianceTotalPoints = parseInt(document.getElementById('allianceTotalPoints').value) || 0;
    const teleopContribution = parseInt(document.getElementById('teleopContribution').value);
    
    if (teleopScored > 0 && allianceTotalPoints === 0) {
        warnings.push("Teleop: Alliance Total Points is 0 but robot scored fuel (Teleop tab).");
    }
    
    if (allianceTotalPoints > 0 && teleopScored > allianceTotalPoints) {
        required.push("Teleop: Robot's Fuel Scored (" + teleopScored + ") cannot exceed Alliance Total Points (" + allianceTotalPoints + ") (Teleop tab).");
    }

    // ═══ EXISTING VALIDATIONS ═══
    const disconnectChecked = document.querySelector('input[name="disconnect"]:checked');
    if (!disconnectChecked) {
        warnings.push("Disconnect status not selected (Endgame tab).");
    }

    const activeRoleAny = document.getElementById('activeRoleFunnel').checked ||
        document.getElementById('activeRoleScoring').checked ||
        document.getElementById('activeRoleDeliver').checked ||
        document.getElementById('activeRoleDefend').checked ||
        document.getElementById('activeRoleNoMove').checked ||
        document.getElementById('activeRoleRefill').checked;
    if (!activeRoleAny) {
        warnings.push("No Active Phase Role selected (Teleop tab).");
    }

    const inactiveRoleAny = document.getElementById('inactiveRoleLimit').checked ||
        document.getElementById('inactiveRoleFunnel').checked ||
        document.getElementById('inactiveRoleDeliver').checked ||
        document.getElementById('inactiveRoleBlock').checked ||
        document.getElementById('inactiveRoleRefill').checked ||
        document.getElementById('inactiveRoleNoMove').checked;
    if (!inactiveRoleAny) {
        warnings.push("No Inactive Phase Role selected (Teleop tab).");
    }

    const transitionRoleAny = document.getElementById('transitionRoleScoring').checked ||
        document.getElementById('transitionRoleLimit').checked ||
        document.getElementById('transitionRoleFunnel').checked ||
        document.getElementById('transitionRoleDeliver').checked ||
        document.getElementById('transitionRoleBlock').checked ||
        document.getElementById('transitionRoleRefill').checked ||
        document.getElementById('transitionRoleNoMove').checked;
    if (!transitionRoleAny) {
        warnings.push("No Transition Phase Role selected (Teleop tab).");
    }

    const traversalChecked = document.querySelector('input[name="traversal"]:checked');
    if (!traversalChecked) {
        warnings.push("Traversal type not selected (Teleop tab).");
    }

    const notes = document.getElementById('endgameNotes').value.trim();
    if (!notes) {
        warnings.push("Notes are empty (Endgame tab).");
    }

    if (required.length > 0 || warnings.length > 0) {
        showValidationModal(required, warnings, onValid);
        return;
    }

    onValid();
}
function validatePit(onValid) {
    const required = [];
    const warnings = [];

    var asciiErrors = checkNonAscii([
        { id: "pitStrategy",   label: "General Strategy" },
        { id: "pitBreakdowns", label: "Breakdowns" },
        { id: "pitNotes",      label: "Notes" }
    ]);
    for (var i = 0; i < asciiErrors.length; i++) {
        required.push(asciiErrors[i]);
    }

    const scoutName = document.getElementById('userName').value.trim();
    if (!scoutName) {
        required.push("Scout name is required.");
    } else if (!/^[a-zA-Z\s]+$/.test(scoutName)) {
        required.push("Scout name should only contain letters — no numbers or special characters.");
    }

    const teamNumVal = document.getElementById('teamPitNum').value.trim();
    if (!teamNumVal) {
        required.push("Team number is required.");
    } else if (parseInt(teamNumVal) > 12000) {
        required.push("Team number cannot exceed 12000.");
    }

    const heightVal = document.getElementById('pitHeight').value.trim();
    if (!heightVal) {
        warnings.push("Height is empty.");
    } else if (parseFloat(heightVal) > 30) {
        required.push("Height cannot exceed 30 inches.");
    }

    const weightVal = document.getElementById('pitWeight').value.trim();
    if (!weightVal) {
        warnings.push("Weight is empty.");
    } else if (parseFloat(weightVal) > 115) {
        required.push("Weight cannot exceed 115 lbs.");
    }

    if (!document.getElementById('pitClimbLevel').value) {
        warnings.push("Climb Level not selected.");
    }

    if (!document.getElementById('pitLauncherType').value) {
        warnings.push("Launcher Type not selected.");
    }

    if (!document.getElementById('pitDriveMechanism').value) {
        warnings.push("Drive Mechanism not selected.");
    }

    if (!document.getElementById('pitDriveExperience').value) {
        warnings.push("Drive Experience not selected.");
    }

    if (!document.getElementById('pitHopperType').value) {
        warnings.push("Hopper Type not selected.");
    }

    if (!document.getElementById('pitStrategy').value.trim()) {
        warnings.push("General Strategy is empty.");
    }

    const outsidePerimeterChecked = document.querySelector('input[name="Outside Perimeter"]:checked');
    if (!outsidePerimeterChecked) {
        warnings.push("Outside Perimeter not selected.");
    }

    if (!document.getElementById('pitNotes').value.trim()) {
        warnings.push("Notes are empty.");
    }

    if (required.length > 0 || warnings.length > 0) {
        showValidationModal(required, warnings, onValid);
        return;
    }

    onValid();
}

// Generate the CSV array for a match
function generateMatchCSV(){
    const data = collectMatchData();

    // TELEOP ACTIVE
    const teleopActiveRoleArr = [
        data.teleop.activeRole.funnel ? "Funnel to Teammates" : null,
        data.teleop.activeRole.score ? "Score" : null,
        data.teleop.activeRole.deliver ? "Deliver to HP" : null,
        data.teleop.activeRole.defend ? "Defend" : null,
        data.teleop.activeRole.noMove ? "Didn't move" : null,
        data.teleop.activeRole.refillHopper ? "Refill Hopper" : null
    ];
    
    let teleopActiveRole = "";
    
    for(let i = 0; i < teleopActiveRoleArr.length; i++){
        if(teleopActiveRoleArr[i] != null){
            teleopActiveRole += teleopActiveRoleArr[i] + ",";
        }
    }

    // Remove trailing comma if it exists
    if(teleopActiveRole.endsWith(",")){
        teleopActiveRole = teleopActiveRole.slice(0, -1);
    }
    
    // TELEOP INACTIVE
    const teleopInactiveRoleArr = [
        data.teleop.inactiveRole.limitingBalls ? "Limiting Balls" : null,
        data.teleop.inactiveRole.funnel ? "Funnel to Teammates" : null,
        data.teleop.inactiveRole.deliver ? "Deliver to HP" : null,
        data.teleop.inactiveRole.blocking ? "Blocking" : null,
        data.teleop.inactiveRole.refillHopper ? "Refill Hopper" : null,
        data.teleop.inactiveRole.noMove ? "Didn't move" : null
    ];

    let teleopInactiveRole = "";
    
    for(let i = 0; i < teleopInactiveRoleArr.length; i++){
        if(teleopInactiveRoleArr[i] != null){
            teleopInactiveRole += teleopInactiveRoleArr[i] + ",";
        }
    }

    // Remove trailing comma if it exists
    if(teleopInactiveRole.endsWith(",")){
        teleopInactiveRole = teleopInactiveRole.slice(0, -1);
    }
    
    // TELEOP TRANSITION
    const teleopTransitionRoleArr = [
        data.teleop.transitionRole.score ? "Score" : null,
        data.teleop.transitionRole.limitingBalls ? "Limiting Balls" : null,
        data.teleop.transitionRole.funnel ? "Funnel to Teammates" : null,
        data.teleop.transitionRole.deliver ? "Deliver to HP" : null,
        data.teleop.transitionRole.blocking ? "Blocking" : null,
        data.teleop.transitionRole.refillHopper ? "Refill Hopper" : null,
        data.teleop.transitionRole.noMove ? "Didn't move" : null
    ];

    let teleopTransitionRole = "";
    
    for(let i = 0; i < teleopTransitionRoleArr.length; i++){
        if(teleopTransitionRoleArr[i] != null){
            teleopTransitionRole += teleopTransitionRoleArr[i] + ",";
        }
    }

    // Remove trailing comma if it exists
    if(teleopTransitionRole.endsWith(",")){
        teleopTransitionRole = teleopTransitionRole.slice(0, -1);
    }

    let traversalType = "";
    if(data.teleop.bumpTraversal){
        traversalType = "Bump";
    }
    else if(data.teleop.trenchTraversal){
        traversalType = "Trench";
    }
    else if(data.teleop.bothTraversal){
        traversalType = "Both";
    }
    else if(data.teleop.noTraversal){
        traversalType = "Didn't move";
    }

    let matchType = "";
    if(data.qualMatch){
        matchType = "Q";
    }
    else if(data.playoffMatch){
        matchType = "P";
    }

    let totalMatch = matchType + data.matchNumber;
    
    const humanPlayerRoles = [
        data.humanPlayer.fedRobot ? "Fed balls to Robot" : null,
        data.humanPlayer.fedHumanPlayers ? "Fed balls to Human Players" : null
    ];

    let autoClimbLevel = "";

    if(data.auto.climbLevel == "Level 1"){
        autoClimbLevel = "1";
    }
    else{
        autoClimbLevel = "0";
    }
    
    let endgameClimbLevel = "";

    if(data.endgame.climbLevel.startsWith("Level")){
        endgameClimbLevel = "L"+data.endgame.climbLevel.substring(6);
    }
    else if(data.endgame.climbLevel == "None"){
        endgameClimbLevel = "None";
    }
    else{
        endgameClimbLevel = "Failed";
    }

    const values = [

        // General  
        data.teamNumber,
        data.event,
        totalMatch,
        data.driverStation,
        data.scoutName,

        // Auto Scouting
        data.auto.fuelScored,
        data.auto.fuelMissed,
        autoClimbLevel,
        "",

        // Teleop Scouting
        data.teleop.fuelScored,
        data.teleop.fuelMissed,
        
        data.humanPlayer.fuelScored,
        data.humanPlayer.fuelMissed,
        teleopActiveRole,
        teleopInactiveRole,
        teleopTransitionRole,
        endgameClimbLevel,
        data.teleop.fouls,

        data.teleop.drivingScore,
        data.teleop.collaborationScore,
        data.endgame.disconnect,
        data.endgame.climbSpeed,
        data.endgame.notes,
        data.auto.ballsCollectedToOurSide,
        traversalType,
        humanPlayerRoles,
        data.humanPlayer.HPnotes
    ]

    for(let i = 0; i < values.length; i++){
        values[i] = '"' + values[i] + '"';
    }

    return values.join(',');
}

// Generate the CSV array for a pit scouting
function generatePitCSV(){
    const pitData = collectPitData();
    
    let traversalTypePit = "";
    if(pitData.bumpTraversalPit){
        traversalTypePit = "Bump";
    }
    else if(pitData.trenchTraversalPit){
        traversalTypePit = "Trench";
    }
    else if(pitData.bothTraversalPit){
        traversalTypePit = "Both";
    }

    const featuresArr = [
        pitData.features.vision ? "Vision" : null,
        pitData.features.adjustableScoring ? "Adjustable Scoring" : null
    ];
    
    let features = "";
    
    for(let i = 0; i < featuresArr.length; i++){
        if(featuresArr[i] != null){
            features += featuresArr[i] + ",";
        }
    }

    // Remove trailing comma if it exists
    if (features.endsWith(",")) {
        features = features.slice(0, -1);
    }

    const intakeTypeArr = [
        pitData.intakeType.overTheBumper ? "Over The Bumper" : null,
        pitData.intakeType.inPerimeter ? "In Perimeter" : null,
        pitData.intakeType.stationIntake ? "Station Intake" : null
    ];
    
    let intakeType = "";
    
    for(let i = 0; i < intakeTypeArr.length; i++){
        if(intakeTypeArr[i] != null){
            intakeType += intakeTypeArr[i] + ",";
        }
    }

    // Remove trailing comma if it exists
    if(intakeType.endsWith(",")){
        intakeType = intakeType.slice(0, -1);
    }

    const teamName = getTeamName(pitData.teamNumber);

    const values = [
        // Pit Scouting
        pitData.teamNumber,
        teamName,
        pitData.climbLevel,
        pitData.heightInches,
        pitData.weightLbs,
        intakeType,
        traversalTypePit,
        pitData.launcherType,
        pitData.driveMechanism,
        pitData.strategy,
        pitData.auto,
        features,
        pitData.hopperType,
        pitData.hopperCapacity,
        pitData.driveExperience,
        pitData.buildQuality,
        pitData.outsidePerimeter,
        pitData.breakdowns,
        pitData.notes,
        pitData.scoutName
    ]

    for(let i = 0; i < values.length; i++){
        values[i] = '"' + values[i] + '"';
    }

    return values.join(',');
}

// Create object of all match data points being collected
function collectMatchData(){

    const data = {
        // Match info
        event: document.getElementById('event').value,
        qualMatch: document.getElementById('qualification').checked,
        playoffMatch: document.getElementById('playoff').checked,
        matchNumber: document.getElementById('matchNumber').value,
        teamNumber: document.getElementById('teamNumber').value,
        scoutName: document.getElementById('userName').value,
        driverStation: document.getElementById('driverStation').value,

        auto: {
            climbLevel: document.getElementById('autoClimbLevel').value,
            fuelScored: document.getElementById('autoFuelScored').value,
            fuelMissed: document.getElementById('autoFuelMissed').value,
            ballsCollectedToOurSide: document.getElementById('autoBallsCollected').checked,
        },        

        teleop: {
            fuelScored: document.getElementById('teleopFuelScored').value,
            fuelMissed: document.getElementById('teleopFuelMissed').value,

            bumpTraversal: document.getElementById('Bump').checked,
            trenchTraversal: document.getElementById('Trench').checked,
            bothTraversal: document.getElementById('Both').checked,
            noTraversal: document.getElementById('No').checked,

            activeRole: {
                funnel: document.getElementById('activeRoleFunnel').checked,
                score: document.getElementById('activeRoleScoring').checked,
                deliver: document.getElementById('activeRoleDeliver').checked,
                defend: document.getElementById('activeRoleDefend').checked,
                noMove: document.getElementById('activeRoleNoMove').checked,
                refillHopper: document.getElementById('activeRoleRefill').checked,

            },

            inactiveRole: {
                limitingBalls: document.getElementById('inactiveRoleLimit').checked,
                funnel: document.getElementById('inactiveRoleFunnel').checked,
                deliver: document.getElementById('inactiveRoleDeliver').checked,
                blocking: document.getElementById('inactiveRoleBlock').checked,
                refillHopper: document.getElementById('inactiveRoleRefill').checked,
                noMove: document.getElementById('inactiveRoleNoMove').checked

            },

            transitionRole: {
                limitingBalls: document.getElementById('transitionRoleLimit').checked,
                score: document.getElementById('transitionRoleScoring').checked,
                deliver: document.getElementById('transitionRoleDeliver').checked,
                blocking: document.getElementById('transitionRoleBlock').checked,
                funnel: document.getElementById('transitionRoleFunnel').checked,
                refillHopper: document.getElementById('transitionRoleRefill').checked,
                noMove: document.getElementById('transitionRoleNoMove').checked
            },

            fouls: parseInt(document.getElementById('teleopFouls').textContent),

            collaborationScore: document.getElementById('teleopCollaboration').value,
            drivingScore: document.getElementById('drivingScore').value,
        },

        endgame: {
            climbLevel: document.getElementById('endgameClimbLevel').value,
            climbSpeed: document.getElementById('endgameClimbSpeed').value,
            
            disconnect: document.querySelector('input[name="disconnect"]:checked')?.value || "",           
            notes: document.getElementById('endgameNotes').value,
        },

        humanPlayer: {
            fuelMissed: parseInt(document.getElementById('humanPlayerFuelMissed').textContent),
            fuelScored: parseInt(document.getElementById('humanPlayerFuelScored').textContent),
        
            fedRobot: document.getElementById('fedRobot').checked,
            fedHumanPlayers: document.getElementById('fedHumanPlayers').checked,
        
           HPnotes: (document.getElementById('humanPlayerNotes') || document.getElementById('endgameNotes') || {value:''}).value
        }
        
    }
    return data;
}

// Create object of all pit data points being collected
function collectPitData(){
    const pitData = {
        scoutName: document.getElementById('userName').value,
        teamNumber: document.getElementById('teamPitNum').value,
        climbLevel: document.getElementById('pitClimbLevel').value,
        heightInches: document.getElementById('pitHeight').value,
        weightLbs: document.getElementById('pitWeight').value,
        bumpTraversalPit: document.getElementById('BumpTrav').checked,
        trenchTraversalPit: document.getElementById('TrenchTrav').checked,
        bothTraversalPit: document.getElementById('BothTrav').checked,
        intakeType: {
            overTheBumper: document.getElementById('otbIntake').checked,
            inPerimeter: document.getElementById('perimeterIntake').checked,
            stationIntake: document.getElementById('stationIntake').checked
        },
        launcherType: document.getElementById('pitLauncherType').value,
        driveMechanism: document.getElementById('pitDriveMechanism').value,
        strategy: document.getElementById('pitStrategy').value,
        auto: document.getElementById('autoStrategy').value,

        features: {
            vision: document.getElementById('pitVision').checked,
            adjustableScoring: document.getElementById('pitAdjustable').checked
        },
        hopperType: document.getElementById('pitHopperType').value,
        hopperCapacity: document.getElementById('pitHopperCapacity').value,
        driveExperience: document.getElementById('pitDriveExperience').value,
        buildQuality: document.getElementById('pitBuildQuality').value,
        outsidePerimeter: document.querySelector('input[name="Outside Perimeter"]:checked')?.value || "",             
        breakdowns: document.getElementById('pitBreakdowns').value,
        notes: document.getElementById('pitNotes').value
    }

    return pitData;
}

function downloadCSV(csvContent, filename) {
    const blob = new Blob(["\uFEFF", csvContent], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);

    const link = document.createElement('a');
    link.href = url;
    link.download = filename;
    link.click();

    // Clean up
    URL.revokeObjectURL(url);
}

let count = Number(localStorage.getItem("MatchCount")) || 0;

function saveMatch() {
    validateMatch(function() {
        const csv = generateMatchCSV();
        downloadCSV(csv, new Date().toISOString() + ".csv");

        localStorage.setItem("Match" + count, csv);
        count++;

        localStorage.setItem("MatchCount", count);

        updateMatchCount();
        pastMatchesQR();
        resetForm();
    });
}

let pitCount = Number(localStorage.getItem("PitCount")) || 0;

function savePit(){
    validatePit(function() {
        const csv = generatePitCSV();
        downloadCSV(csv, new Date().toISOString() + ".csv");

        localStorage.setItem("Pit" + pitCount, csv);
        pitCount++;

        localStorage.setItem("PitCount", pitCount);

        updatePitCount();
        pastPitsQR();
        resetForm();
    });
}

function pastMatchesQR() {
    const matchContent = document.getElementById("matchContent");
    matchContent.innerHTML = "";
    const totalMatches = Number(localStorage.getItem("MatchCount")) || 0;

    for (let i = 0; i < totalMatches; i++) {
        const csv = localStorage.getItem("Match" + i);
        if (!csv) continue;

        let tempArr = parseCSVRow(csv);

        const matchDiv = document.createElement("div");
        matchDiv.style.cssText = "display:flex; gap:6px;";
        
        const matchButton = document.createElement("button");
        matchButton.textContent = tempArr[0] + " -- " + tempArr[2];
        matchButton.className = "match-button";
        matchButton.style.flex = "1";
        matchButton.onclick = ((data) => () => openQRModal(data, "Match QR"))(csv);
        
        const editBtn = document.createElement("button");
        editBtn.textContent = "✏️";
        editBtn.className = "match-button";
        editBtn.style.cssText = "width:50px; flex-shrink:0;";
        editBtn.onclick = ((idx, data, label) => () => openEditModal(idx, data, label))(i, csv, tempArr[0] + " -- " + tempArr[2]);
        
        matchDiv.appendChild(matchButton);
        matchDiv.appendChild(editBtn);
        matchContent.appendChild(matchDiv);
    }
}

let editingIndex = null;
let editingType = 'match';
let _pendingSaveCallback = null;

function openEditModal(index, csv, title, type = 'match') {
    editingIndex = index;
    editingType = type;
    document.getElementById('editModalTitle').textContent = 'Edit: ' + title;
    document.getElementById('editCSV').value = csv;
    document.getElementById('editModal').classList.add('active');
}

function closeEditModal() {
    document.getElementById('editModal').classList.remove('active');
    editingIndex = null;
}

function saveEdit() {
    const newCSV = document.getElementById('editCSV').value.trim();
    
    if (!newCSV) {
        alert('CSV cannot be empty');
        return;
    }
    
    const key = editingType === 'match' ? "Match" : "Pit";
    localStorage.setItem(key + editingIndex, newCSV);
    
    closeEditModal();
    
    if (editingType === 'match') {
        pastMatchesQR();
    } else {
        pastPitsQR();
    }
    
    alert('Saved!');
}

function fillTestData() {
    // Match info
    document.getElementById('event').value = 'North Bay';
    document.getElementById('qualification').checked = true;
    document.getElementById('matchNumber').value = Math.floor(Math.random() * 50) + 1;
    document.getElementById('teamNumber').value = Math.floor(Math.random() * 10000) + 1000;
    document.getElementById('driverStation').value = ['R1', 'R2', 'R3', 'B1', 'B2', 'B3'][Math.floor(Math.random() * 6)];
    document.getElementById('userName').value = 'Test Scout';
    
    // Auto
    document.getElementById('autoFuelScored').textContent = Math.floor(Math.random() * 10);
    document.getElementById('autoFuelMissed').textContent = Math.floor(Math.random() * 5);
    document.getElementById('autoClimbLevel').value = ['None', 'Level 1'][Math.floor(Math.random() * 2)];
    document.getElementById('autoBallsCollected').checked = Math.random() > 0.5;
    
    // Teleop
    document.getElementById('teleopFuelScored').textContent = Math.floor(Math.random() * 20);
    document.getElementById('teleopFuelMissed').textContent = Math.floor(Math.random() * 10);
    document.getElementById('humanPlayerFuelMissed').textContent = Math.floor(Math.random() * 15);
    document.getElementById('humanPlayerFuelScored').textContent = Math.floor(Math.random() * parseInt(document.getElementById('humanPlayerFuelMissed').textContent));
    
    // Roles
    document.getElementById('activeRoleFunnel').checked = Math.random() > 0.5;
    document.getElementById('activeRoleScoring').checked = Math.random() > 0.5;
    document.getElementById('inactiveRoleLimit').checked = Math.random() > 0.5;
    document.getElementById('transitionRoleScoring').checked = Math.random() > 0.5;
    
    // Traversal
    const traversals = ['Bump', 'Trench', 'Both'];
    document.getElementById(traversals[Math.floor(Math.random() * 3)]).checked = true;
    
    document.getElementById('teleopFouls').textContent = Math.floor(Math.random() * 3);
    document.getElementById('teleopCollaboration').value = Math.floor(Math.random() * 4) + 1;
    document.getElementById('drivingScore').value = Math.floor(Math.random() * 4) + 1;
    
    // Update slider displays
    document.getElementById('collab-val').textContent = document.getElementById('teleopCollaboration').value;
    document.getElementById('drive-val').textContent = document.getElementById('drivingScore').value;
    
    // Endgame
    document.getElementById('endgameClimbLevel').value = ['None', 'Level 1', 'Level 2', 'Level 3'][Math.floor(Math.random() * 4)];
    document.getElementById('endgameClimbSpeed').value = ['N/A', 'Slow', 'Medium', 'Fast'][Math.floor(Math.random() * 4)];
    document.getElementById('disconnectNo').checked = true;
    document.getElementById('endgameNotes').value = 'Auto-generated test data';
    
    alert('✓ Form filled with test data!');
}

function deleteFromEdit() {
    if (confirm('Delete this? Cannot be undone.')) {
        const key = editingType === 'match' ? "Match" : "Pit";
        const countKey = editingType === 'match' ? "MatchCount" : "PitCount";
        
        // Get current count
        let currentCount = Number(localStorage.getItem(countKey)) || 0;
        
        // Remove the item
        localStorage.removeItem(key + editingIndex);
        
        // Shift all items after this one down
        for (let i = editingIndex + 1; i < currentCount; i++) {
            const item = localStorage.getItem(key + i);
            if (item) {
                localStorage.setItem(key + (i - 1), item);
                localStorage.removeItem(key + i);
            }
        }
        
        // Update count
        localStorage.setItem(countKey, currentCount - 1);
        
        closeEditModal();
        
        // Refresh without reload
        if (editingType === 'match') {
            count = currentCount - 1; // Update global count
            updateMatchCount();
            pastMatchesQR();
        } else {
            pitCount = currentCount - 1; // Update global pitCount
            updatePitCount();
            pastPitsQR();
        }
    }
}

function pastPitsQR() {
    const pitContent = document.getElementById("pitContent");
    pitContent.innerHTML = "";
    const totalPits = Number(localStorage.getItem("PitCount")) || 0;

    for (let i = 0; i < totalPits; i++) {
        const csv = localStorage.getItem("Pit" + i);
        if (!csv) continue;

        let tempArr = parseCSVRow(csv);

        const pitDiv = document.createElement("div");
        pitDiv.style.cssText = "display:flex; gap:6px;";
        
        const pitButton = document.createElement("button");
        pitButton.textContent = "Pit " + tempArr[0];
        pitButton.className = "match-button";
        pitButton.style.flex = "1";
        pitButton.onclick = ((data) => () => openQRModal(data, "Pit QR"))(csv);
        
        const editBtn = document.createElement("button");
        editBtn.textContent = "✏️";
        editBtn.className = "match-button";
        editBtn.style.cssText = "width:50px; flex-shrink:0;";
        editBtn.onclick = ((idx, data, label) => () => openEditModal(idx, data, "Pit " + label, 'pit'))(i, csv, tempArr[0]);
        
        pitDiv.appendChild(pitButton);
        pitDiv.appendChild(editBtn);
        pitContent.appendChild(pitDiv);
    }
}


function openQRModal(text, title) {
    const modal = document.getElementById("qrModal");
    const qrTitle = document.getElementById("qrModalTitle");
    const qrCodeDiv = document.getElementById("qrcode");

    qrTitle.textContent = title;
    qrCodeDiv.innerHTML = "";

    const size = Math.min(window.innerWidth * 0.8, 450);

    new QRCode(qrCodeDiv, {
        text: text,
        width: size,
        height: size,
        colorDark: "#da4416",
        colorLight: "#ffffff",
        correctLevel: QRCode.CorrectLevel.L,
    });

    modal.classList.add("active");
}

function closeQRModal() {
    const modal = document.getElementById("qrModal");
    const qrCodeDiv = document.getElementById("qrcode");
    modal.classList.remove("active");
    qrCodeDiv.innerHTML = "";
    const existing = document.getElementById("modalDeleteBtn");
    if (existing) existing.remove();
}

window.onload = function () {
    updateMatchCount();
    updatePitCount();
    pastMatchesQR();
    pastPitsQR();
};

function updateMatchCount() {
    const count = Number(localStorage.getItem("MatchCount")) || 0;
    document.getElementById("matchHistoryCount").textContent = count;
}

function updatePitCount() {
    const count = Number(localStorage.getItem("PitCount")) || 0;
    document.getElementById("pitHistoryCount").textContent = count;
}

function exportAllPitCSV(){
    let totalPit = "";
    const pitCount = Number(localStorage.getItem("PitCount")) || 0;

    for(let i = 0; i < pitCount; i++){
        const pit = localStorage.getItem("Pit" + i);
        if (pit) {
            totalPit += pit + "\n";
        }
    }

    downloadCSV(totalPit, "ALL_PITS");
    openQRModal(totalPit, "ALL PITS");
}

function exportAllMatchCSV(){
    let totalMatch = "";
    const matchCount = Number(localStorage.getItem("MatchCount")) || 0;

        for(let i = 0; i < matchCount; i++){
        const match = localStorage.getItem("Match" + i);
        if (match) {
            totalMatch += match + "\n";
        }
    }

    downloadCSV(totalMatch, "ALL_MATCHES");
    openQRModal(totalMatch, "ALL MATCHES");
}

function clearHistory(){
    localStorage.clear();
    window.location.reload();
}